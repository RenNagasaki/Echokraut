...
namespace Echokraut.Helper.Functional
{
    public static class AlltalkInstanceHelper
    {
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(Plugin.Configuration.Alltalk.CustomModelUrl))
                {
                    ...
                    using (var client = new HttpClient())
                    {
                        try
                        {
                            ...
                            var downloadUrl =
                                CheckForGoogleAndConvertToDirectDownloadLink(
                            ...
                            if (isGoogle)
                                response = DownloadGoogleDrive(downloadUrl, response, client);
                ...
                if (!string.IsNullOrWhiteSpace(Plugin.Configuration.Alltalk.CustomVoicesUrl))
                {
                    ...
                    using (var client = new HttpClient())
                    {
                        try
                        {
                            var downloadUrl = CheckForGoogleAndConvertToDirectDownloadLink(Plugin.Configuration.Alltalk.CustomVoicesUrl, out bool isGoogle);
                            LogHelper.Debug("InstallCustomData",
                                            $"{downloadUrl}", eventId);
                            var response = await client.GetAsync(downloadUrl);
        ...
        private static HttpResponseMessage DownloadGoogleDrive(string downloadUrl, HttpResponseMessage response, HttpClient client)
        {
