...
public interface IDatabaseService : IDisposable
{
    ...
    /// <summary>
    /// The phonetic rules pre-projected into the model the text pipeline takes. Use this on the
    /// live path — it is served from cache, whereas projecting <see cref="GetPhoneticCorrections"/>
    /// rebuilds the list on every call, and the speech pipeline needs it for every spoken line.
    /// </summary>
    List<PhoneticCorrection> GetPhoneticCorrectionModels();

    List<CharacterEntity> GetAllCharacters();

    /// <summary>
    /// Single character row by primary key, without materialising the whole table. Use this on
    /// the live dialogue path — <see cref="GetAllCharacters"/> is uncached and reads everything.
    /// </summary>
    CharacterEntity? FindCharacterById(int characterId);

    CharacterEntity? FindCharacter(string name, Genders gender, NpcRaces race, int language);

    /// <summary>
    /// Sets (or with <c>null</c>, clears) the user's gender/race correction for one character. The
    /// ONLY writer of those two columns: everything else runs off game-reported data and would erase
    /// the correction. Returns false when the row no longer exists.
    /// </summary>
    bool SetIdentityOverride(int characterId, int? genderOverride, int? raceOverride);
    ...
    /// <summary>
    /// One-shot cleanup for rows written before the live path tokenised player names: rewrites
    /// the literal player name back into <c>-PlayerName-</c> / <c>-PlayerFirstName-</c> /
    /// <c>-PlayerLastName-</c> and merges each row into its harvested twin when tokenising makes
    /// the two identical (generation rows follow, the duplicate goes). No file on disk is
    /// touched — both spellings always hashed to the same WAV name. Pass
    /// <paramref name="dryRun"/> to preview.
    /// </summary>
    PlayerNameRetokenizeResult RetokenizePlayerNames(string playerName, bool dryRun,
        System.Threading.CancellationToken ct = default);
    /// <summary>One stable page of a language's harvested lines, ordered by id, for the bulk
    /// import. See the implementation for why the order is by id and not by time.</summary>
    List<VoiceClipEntity> GetVoiceClipsForLanguage(int language, int offset, int limit);

    /// <summary>How many lines the bulk import has to walk for a language.</summary>
    int GetVoiceClipCountForLanguage(int language);
    ...
    /// <summary>
    /// Flat join of every <c>voice_clip_generation</c> with its clip and character, optionally
    /// restricted to one game language. One round-trip, no change tracking — the audio package
    /// exporter needs the whole set at once and never writes any of it back.
    /// Personal rows are NOT filtered here; that judgement lives in
    /// <c>Helper.Functional.AudioPackageRules.IsShareable</c>.
    /// </summary>
    List<AudioPackageExportRow> GetAudioPackageExportRows(int? language = null);
