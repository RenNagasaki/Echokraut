...
public interface IEchokrauTtsInstanceService
{
    ...
    /// <summary>
    /// Developer-only: whether the test-install button may be shown, i.e. a locally built wrapper zip
    /// lies at <c>TestWrapperInstall.ZipPath</c> — an absolute path inside a source checkout, so this
    /// is false on every installed copy of the plugin. Asked per frame by the UI and therefore cached
    /// behind a short re-probe interval; see <see cref="InstallTestWrapper"/> for why this exists.
    /// </summary>
    bool TestInstallAvailable { get; }

    /// <summary>
    /// Developer-only: install the wrapper from that local zip instead of a published release, then
    /// behave exactly like <see cref="Install"/> (wholesale unpack, voice pack seeded, marked
    /// installed). Exists because the real install path is otherwise untestable before a release:
    /// the download URL is read from <c>RemoteUrls.json</c> on the plugin's <c>main</c> branch, which
    /// overrides the embedded copy, and the update check uses GitHub's <c>/releases/latest</c>, which
    /// ignores drafts and pre-releases. Records <c>TestWrapperInstall.VersionMarker</c> as the
    /// installed tag rather than pretending to be a release. Runs off the UI thread.
    /// </summary>
    void InstallTestWrapper();
    ...
    /// <summary>
    /// Sub-engines the wrapper knows about, as published by the wrapper repo — the source the engine
    /// dropdown is built from. Exposed here (rather than injecting the catalog service into every
    /// window) because the same service owns the installed wrapper version this list is judged
    /// against. Never empty: it falls back to the list embedded in the plugin.
    /// </summary>
    IReadOnlyList<TtsEngineInfo> AvailableEngines { get; }

    /// <summary>
    /// Change the local sub-engine by its wrapper id (<c>xtts</c>/<c>f5</c>/<c>moss</c>/…). Persists
    /// the choice; if a local instance is currently running or starting, restarts it so the wrapper
    /// reloads with the new <c>--tts-backend</c> (the bootstrap installs all engines, so this is a
    /// restart, not a reinstall). No-op when the engine is unchanged or the id is blank.
    /// </summary>
    void SwitchTtsBackend(string engineId);
