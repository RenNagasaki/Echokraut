...
public interface INpcDataService
{
    ...
    /// <summary>
    /// Saves a USER edit of gender/race as an override, leaving the game-reported identity (and thus
    /// the lookup key) untouched. Use this and never <see cref="SaveCharacterWithOldIdentity"/> for
    /// edits made in the UI — re-keying on a user's opinion orphans the row, and the next line
    /// recreates the original. <c>null</c> clears a correction.
    /// </summary>
    void SaveCharacterWithIdentityOverride(NpcMapData data, Genders? genderOverride, NpcRaces? raceOverride);
