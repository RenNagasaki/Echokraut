...
public class AlltalkInstanceService : IAlltalkInstanceService, IDisposable
{
    ...
    public AlltalkInstanceService(ILogService log, Configuration config, IGoogleDriveSyncService googleDrive,
        IRemoteUrlService remoteUrls, IClientState clientState, IVoicePackService voicePack)
    {
    ...
    public void Install()
    {
        ...
        try
        {
            ...
            _installThread = Task.Run(() =>
            {
                ...
                // Seed the install with the curated, freely-licensed voice pack. Output target is
                // AllTalk's canonical voices folder (alltalk_tts/voices/), which the pack service
                // wipes before unpacking so leftovers from a previous run don't haunt the voice list.
                // When the user supplied their own custom voices, the installer already extracted
                // them into alltalk_tts/voices/ during the install step. Unpacking the pack here
                // would WIPE that folder and replace the custom voices — so skip it.
                if (string.IsNullOrWhiteSpace(_config.Alltalk.CustomVoicesUrl))
                {
                    ...
                    _log.Info(nameof(Install), $"Downloading voice pack into {voicesDir}...", eventId);
                    CurrentInstallStatus = "Downloading voices...";
                    CurrentInstallProgress = 0.50f;
                    // Subscribe to pack progress for the duration of this run so the First-Time
                    // window's progress bar shows real granularity instead of the indeterminate
                    // sticky-50% during the opaque installer phase. Mapping: 0..1 → 0.50..0.95.
                    ...
                    try
                    {
                        ...
                        _log.Info(nameof(Install), "Voice pack ready.", eventId);
                    }
                    catch (Exception ex)
                    {
                        // Non-fatal: install still completes. User can re-run the download from
                        // the Game Data Tools window if voices are missing.
