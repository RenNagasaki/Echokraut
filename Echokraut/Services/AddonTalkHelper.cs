...
public unsafe class AddonTalkHelper : IAddonTalkHelper
{
    ...
    private void HookIntoFrameworkUpdate()
    {
        ...
        // Second close signal. OnPostUpdate only sees the close while the addon still receives
        // updates — when Talk is torn down outright (cutscene end, zoning, ESC) that never
        // happens, InDialog stays true and queued lines keep playing after the box is gone.
        _addonLifecycle.RegisterListener(AddonEvent.PreFinalize, "Talk", OnPreFinalize);
    }
    ...
    private void OnPreReceiveEvent(AddonEvent type, AddonArgs args)
    {
        ...
        if (eventType == AtkEventType.MouseClick && _dialogState.IsInsideOwnedWindow != null
            && _dialogState.IsInsideOwnedWindow(new System.Numerics.Vector2(eventData->MouseData.PosX, eventData->MouseData.PosY)))
            return;

        if (isControllerButtonClick || isDialogueAdvancing)
            if (_configuration.CancelSpeechOnTextAdvance)
                _cancelService.Cancel(_dialogState.CurrentVoiceMessage);
    }
    ...
    private unsafe void OnPostUpdate(AddonEvent type, AddonArgs args)
    {
        ...
        if (addonTalk != null)
        {
            ...
            if (!visible && wasTalking)
                HandleAddonClosed(nameof(OnPostUpdate));
        }
    }
    ...
    {
        if (wasTalking)
            HandleAddonClosed(nameof(OnPreFinalize));
    }

    /// <summary>
    /// Single close path for the Talk addon: stop treating us as "in dialog", flush the queued
    /// AddonTalk lines and forget the current speaker. Idempotent — <see cref="wasTalking"/>
    /// guards against the two close signals (PostUpdate / PreFinalize) both firing.
    /// </summary>
    private void HandleAddonClosed(string source)
    {
        _log.Info(source, "Addon closed", new EKEventId(0, TextSource.AddonTalk));
        wasTalking = false;
        _audioPlayback.InDialog = false;
        lastValue = new AddonTalkState();
        if (_configuration.CancelSpeechOnTextAdvance)
            _cancelService.Cancel(_dialogState.CurrentVoiceMessage, true);
        _dialogState.CurrentVoiceMessage = null;
        ...
        _dialogState.ClearResolvedSpeakers();
    }
    ...
    public void Dispose()
    {
        ...
        _addonLifecycle.UnregisterListener(AddonEvent.PreFinalize, "Talk", OnPreFinalize);
    }
}
