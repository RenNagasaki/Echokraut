    ...
    sealed class Source : IDisposable
    {
        ...
        void RebufferLoop()
        {
            ...
            while (!_cts.IsCancellationRequested && _h != 0 && !_readerDone)
            {
                ...
                if (!StreamBufferPolicy.ShouldRebuffer(avail, lowWaterByte, _readerDone))
                {
                    Thread.Sleep(20);
                    continue;
                }
                ...
                lock (_stateLock)
                {
                    if (State != PlaybackState.Playing) continue; // user paused/stopped meanwhile
                    if (_h == 0 || !Bass.ChannelPause(_h)) continue;
                    _rebuffering = true;
                }
