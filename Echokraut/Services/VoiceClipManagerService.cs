...
public class VoiceClipManagerService : IVoiceClipManagerService
{
    ...
    /// <summary>
    /// Refuses to send a line to the backend while it still carries an unresolved player-name
    /// placeholder. <see cref="TalkTextHelper.SubstitutePlaceholders"/> silently returns the text
    /// unchanged when the player name is unknown (cold
    /// <see cref="IGameObjectService.LocalPlayerName"/> cache), and the resulting WAV — with the
    /// TTS reading "-PlayerFirstName-" out loud — lands on exactly the on-disk path the live
    /// path adopts later, so a single bad generation keeps being replayed forever. Failing the
    /// generation is cheap; poisoning the audio cache is not.
    /// </summary>
    private bool HasUsableText(string? text, int voiceClipId, string caller, EKEventId eventId)
    {
        if (!TalkTextHelper.ContainsPlayerPlaceholder(text ?? "")) return true;

        _log.Warning(caller,
            $"Refusing to generate voice clip {voiceClipId}: text still contains an unresolved " +
            $"player-name placeholder (local player name is '{_gameObjects.LocalPlayerName}'). " +
            "Log in and try again.", eventId);
        return false;
    }
    ...
    private VoiceMessage BuildVoiceMessageInternal(VoiceClipEntity voiceClip, string? overridePlayerName, bool? overrideIsMale)
    {
        ...
        var npcData = liveCache.FirstOrDefault(n =>
    ...
    public async Task<bool> GenerateForVoiceClip(VoiceClipEntity voiceClip)
    {
        ...
        try
        {
            ...
            if (!HasUsableText(message.Text, voiceClip.Id, nameof(GenerateForVoiceClip), eventId))
                return false;
    ...
    private async Task GenerateAliasVariant(VoiceClipEntity voiceClip, bool isMale)
    {
        ...
        try
        {
            ...
            // Alias variants substitute a localized noun instead of the player name, so they are
            // not affected by a cold name cache — but the same guard still applies if a clip
            // carries a placeholder shape the substitution doesn't know.
            if (!HasUsableText(message.Text, voiceClip.Id, nameof(GenerateAliasVariant), eventId))
                return;
