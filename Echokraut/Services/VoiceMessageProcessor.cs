...
public class VoiceMessageProcessor : IVoiceMessageProcessor
{
    ...
    private readonly Queue.IVoiceMessageQueue _queue;
    ...
    public VoiceMessageProcessor(
        ...
        Queue.IVoiceMessageQueue queue,
    ...
    public async Task ProcessSpeechAsync(EKEventId eventId, IGameObject? speaker, SeString speakerName, string textValue,
        string worldEnglish = "")
    {
        // Stamped as early as possible — everything below can await, and the player may skip past
        // this line while we're still working on it. Whoever cancels the source bumps the epoch,
        // so this message is then recognised as belonging to a line already left behind.
        var cancelEpoch = _queue.GetEpoch(eventId.TextSource);
        ...
        try
        {
            ...
            // Step 2: Clean and normalize text. Chat lines can revise the language (detected from
            // the text) and the speaker, so both come back out of this call.
            var originalText = textValue;
            var cleaned = await CleanTextAsync(textValue, source, eventId, language, speaker);
            language = cleaned.Language;
            speaker = cleaned.Speaker;
            var cleanText = cleaned.Text;
            ...
            // Step 3: Get or create NPC data. Never null — INpcDataService.GetAddCharacterMapData
            // returns the freshly-added `data` when it finds no existing mapping, so every path
            // yields a character. (There used to be a null check here; it could not fire.)
            var npcData = await GetOrCreateNpcDataAsync(speaker, speakerName, cleanText, source, eventId, worldEnglish);

            // Step 3.5: Build the message shell and publish it as the current dialogue speaker.
            // This deliberately happens BEFORE the skip checks below (muted NPC, no voice
            // assigned, volume 0): the in-dialog toolbar reads _dialogState.CurrentVoiceMessage
            // to know who is talking. Publishing only for lines that actually play was a
            // catch-22 — an NPC with no usable voice offered no speaker, so the toolbar's voice
            // dropdown stayed empty and the user could never assign the voice that would have
            // made the line play (same for un-muting a muted NPC). Volume is filled in at
            // step 6 once the voice is known.
            ...
            if (isDialogue)
                _dialogState.CurrentVoiceMessage = voiceMessage;

            // Step 3.5b: offer the line to the community database. Deliberately HERE and not at
            // LogVoiceClip: that sits behind four early returns, two of which fire in None mode —
            // exactly the installations whose reports are most valuable, because they never generate
            // audio and so never learn which lines exist. The call is a cheap enqueue that applies
            // the opt-out, the source whitelist and the local suppression set itself; everything
            // else happens on a timer thread.
            _lineSubmission.Report(npcData, speaker?.BaseId ?? 0, originalText, source, language);
            ...
            // Step 7: Finish the message shell built at step 3.5
            voiceMessage.Volume = finalVolume;
    ...
    /// <summary>
    /// Result of <see cref="CleanTextAsync"/>. Chat lines can change the language (detected from
    /// the text) and the speaker (redirected to the local player when 3D chat is off), which used
    /// to travel back through <c>ref</c> parameters — impossible once the method is async.
    /// </summary>
    private sealed record CleanedText(string? Text, Dalamud.Game.ClientLanguage Language, IGameObject? Speaker);

    private async Task<CleanedText> CleanTextAsync(string textValue, TextSource source, EKEventId eventId, Dalamud.Game.ClientLanguage language, IGameObject? speaker)
    {
        ...
        if (source == TextSource.Chat)
        {
            ...
            language = await _languageDetection.GetTextLanguage(cleanText, eventId).ConfigureAwait(false);
        }
        ...
        cleanText = _textProcessing.ReplacePhonetics(cleanText, _db.GetPhoneticCorrectionModels());
        ...
        _log.Debug(nameof(CleanTextAsync), $"Cleantext: {cleanText}", eventId);

        // Validate result
        if (string.IsNullOrWhiteSpace(cleanText) || !cleanText.Any() || !_textProcessing.IsSpeakable(cleanText))
        {
            _log.Info(nameof(CleanTextAsync), $"Text not speakable: {cleanText}", eventId);
            return new CleanedText(null, language, speaker);
        }

        return new CleanedText(cleanText, language, speaker);
    }
    ...
    ///     (<see cref="_dialogState.SpeakersResolvedThisDialog"/>). FFXIV cutscenes
    ...
    private int? ResolveCharacterByAlias(string fakename, Dalamud.Game.ClientLanguage language, IGameObject? speaker, EKEventId eventId)
    {
        ...
        var notYetSpoken = present.FirstOrDefault(c => !_dialogState.WasSpeakerResolved(c));
    ...
    private async Task<NpcMapData> GetOrCreateNpcDataAsync(IGameObject? speaker, SeString speakerName, string cleanText, TextSource source, EKEventId eventId, string worldEnglish = "")
    {
        ...
        if (npcData.ObjectKind != ObjectKind.Pc)
        {
            ...
            if (string.Equals(beforeJson, npcData.Name, StringComparison.OrdinalIgnoreCase))
            {
                ...
                {
                    var aliasChar = _db.FindCharacterById(resolvedCharId.Value);
                    if (aliasChar != null && !string.IsNullOrEmpty(aliasChar.Name))
                    {
                        ...
                        _dialogState.MarkSpeakerResolved(resolvedCharId.Value);
                    }
                }
            }
        }
    ...
    private void LogVoiceClip(VoiceMessage message)
    {
        try
        {
            ...
            // Re-tokenise before persisting. The game hands us the line with the player's name
            // already substituted; storing it that way created a second, player-specific row
            // beside the harvested one (which carries -PlayerFirstName- & co.), splitting the
            // clip's generation rows across two player_content_id buckets while both rows point
            // at the very same WAV. voice_clips must only ever contain tokens — see
            // PlayerNameTokenizer. No-op while the player name is unknown, and the spoken text
            // (message.Text / message.OriginalText) is deliberately left untouched.
            var playerName = _gameObjects.LocalPlayerName ?? "";
            var originalText = PlayerNameTokenizer.Tokenize(message.OriginalText ?? "", playerName);
            var cleanedText = PlayerNameTokenizer.Tokenize(message.Text ?? "", playerName);
