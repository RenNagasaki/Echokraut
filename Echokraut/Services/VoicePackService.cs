...
public sealed class VoicePackService : IVoicePackService, IDisposable
{
    ...
    // Read per download, not captured once: the user can switch the client language between the
    // first-time install and a later manual re-download from the Game Data Tools window.
    ...
    public VoicePackService(ILogService log, Configuration config, IRemoteUrlService remoteUrls,
        Func<ClientLanguage> language)
        : this(log, config, remoteUrls, language,
               new HttpClient { Timeout = TimeSpan.FromMinutes(30) }, ownsHttp: true)
    {
    }

    public VoicePackService(ILogService log, Configuration config, IRemoteUrlService remoteUrls,
        Func<ClientLanguage> language, HttpClient http, bool ownsHttp = false)
    {
    ...
    {
        // Which pack: the one published for the client's language, else English, else the single
        // pack from before per-language packs existed.
        var languageKey = ClientLanguageKeys.KeyFor(_language());
        var packUrls = _remoteUrls.Urls.VoicePackUrls;
        var url = VoicePackUrlResolver.Resolve(packUrls, languageKey, _remoteUrls.Urls.VoicePackUrl);
        ...
        // Said out loud: in a non-English client without its own pack the voices will sound English,
        // and without this line the user is left guessing why.
        if (VoicePackUrlResolver.IsLanguageSpecific(packUrls, languageKey))
            _log.Info(nameof(RunInternalAsync), $"Using the {languageKey} voice pack.", eventId);
        else
            _log.Info(nameof(RunInternalAsync),
                $"No {languageKey} voice pack published — falling back to the English one.", eventId);
