...
public class BackendService : IBackendService, IDisposable
{
    ...
    public bool IsVoiceFittingForNpc(EchokrautVoice? voice, NpcMapData npc)
    {
        ...
        // Effective*, not the raw values: gender/race are the lookup key and come from the game,
        // but the user may have corrected how this character should SOUND. Voice filtering is
        // exactly where that correction is meant to take effect.
        var isGenderedRace = _npcData.IsGenderedRace(npc.EffectiveRace);
        if (isGenderedRace && voice.AllowedGenders.Count > 0 && !voice.AllowedGenders.Contains(npc.EffectiveGender))
            return false;
        if (voice.AllowedRaces.Count > 0 && !voice.AllowedRaces.Contains(npc.EffectiveRace))
            return false;
    ...
    public bool EnsureFittingVoice(NpcMapData npcData, EKEventId eventId)
    {
        ...
        _log.Info(nameof(EnsureFittingVoice),
            $"Auto-assigned voice '{picked.BackendVoice}' for {npcData.Name} " +
            $"({npcData.EffectiveGender} {npcData.EffectiveRace}, {npcData.BodyType}" +
            $"{(npcData.HasIdentityOverride ? ", user-corrected" : string.Empty)})" +
    ...
    public EchokrautVoice? PickVoice(NpcMapData npcData, IList<EchokrautVoice> voices)
    {
        ...
        // Find by race/gender
        var isGenderedRace = _npcData.IsGenderedRace(npcData.EffectiveRace);
        List<EchokrautVoice>? matches = null;
        for (var i = 0; i < voices.Count; i++)
        {
            // Effective*: see IsSelectable — a user correction must steer the auto-pick too,
            // otherwise "this NPC sounds wrong" is only fixable by hand-picking a voice.
            if (voices[i].FitsNpcData(npcData.EffectiveGender, npcData.EffectiveRace, bodyType, isGenderedRace))
