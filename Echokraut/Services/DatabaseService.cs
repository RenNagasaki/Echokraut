...
public class DatabaseService : IDatabaseService
{
    ...
    /// <summary>
    /// The same phonetic rules already projected into the model the text pipeline consumes.
    /// Cached alongside the entities because <c>VoiceMessageProcessor</c> needs them for EVERY
    /// spoken line and used to rebuild the whole list per line.
    /// </summary>
    private volatile List<PhoneticCorrection> _cachedPhoneticModels = new();
    ...
    private void RunSchemaMigrations()
    {
        ...
        EnsureCharacterGenderRaceOverride();     // v4 → v5
        RecordSchemaVersion();
    }
    ...
    /// <summary>
    /// v5 migration: the two user-override columns on <c>characters</c>. Nullable with no default,
    /// because "the user said nothing" and "the user picked the same value the game reports" have to
    /// stay distinguishable — a sentinel like 0 would collide with a real enum value. Existing rows
    /// get NULL, i.e. unchanged behaviour.
    /// Internal so migration tests can exercise this single step in isolation.
    /// </summary>
    internal void EnsureCharacterGenderRaceOverride()
    {
        if (!TableHasColumn("characters", "gender_override"))
            _context.Database.ExecuteSqlRaw(
                "ALTER TABLE characters ADD COLUMN gender_override INTEGER NULL");
        if (!TableHasColumn("characters", "race_override"))
            _context.Database.ExecuteSqlRaw(
                "ALTER TABLE characters ADD COLUMN race_override INTEGER NULL");
    }
    ...
    /// <summary>
    /// Single character row by primary key. Exists so the live dialogue path doesn't have to go
    /// through <see cref="GetAllCharacters"/> — that one is the only uncached read in this
    /// service and pulls the whole (post-harvest: five-figure) characters table under the write
    /// lock, just to LINQ-scan it for one id.
    /// </summary>
    public CharacterEntity? FindCharacterById(int characterId)
    {
        if (characterId <= 0) return null;
        lock (_writeLock)
        {
            if (_disposed) return null;
            return _context.Characters.AsNoTracking().FirstOrDefault(c => c.Id == characterId);
        }
    }
    ...
    public CharacterEntity UpsertCharacter(CharacterEntity character)
    {
        lock (_writeLock)
        {
            ...
            if (existing != null)
            {
                ...
                // gender_override / race_override are deliberately NOT written here. This runs on
                // every dialogue line with an entity rebuilt from the GAME, which knows nothing
                // about the user's correction — copying the incoming (null) value across would
                // silently erase it on the very next line. SetIdentityOverride is the only writer.
            }
            else
            {
                _context.Characters.Add(character);
    ...
    /// <summary>
    /// The one and only writer of <c>gender_override</c>/<c>race_override</c>. Passing <c>null</c>
    /// clears a correction, which is why the columns are nullable rather than sentinel-valued.
    /// Returns false when the row is gone (deleted underneath an open edit window).
    /// </summary>
    public bool SetIdentityOverride(int characterId, int? genderOverride, int? raceOverride)
    {
        lock (_writeLock)
        {
            if (_disposed) return false;
            var existing = _context.Characters.FirstOrDefault(c => c.Id == characterId);
            if (existing == null) return false;

            existing.GenderOverride = genderOverride;
            existing.RaceOverride = raceOverride;
            _context.SaveChanges();
    ...
    public List<PhoneticCorrection> GetPhoneticCorrectionModels() => _cachedPhoneticModels;
    ...
    /// <summary>
    /// Raised after a voice clip was written. **Always fire this outside <c>_writeLock</c>.**
    /// `NpcDataService.LoadFromDatabase` is subscribed to it and touches its own state; firing
    /// while holding the write lock means a handler runs under our lock, so any handler that
    /// takes a lock of its own creates a lock-order cycle with every thread that takes that lock
    /// first and then calls into this service (which the dialogue path does constantly).
    /// `WipeAll` already got this right; `LogVoiceClip`/`LogOrUpdateVoiceClip` did not.
    /// </summary>
    ...
    public VoiceClipEntity LogOrUpdateVoiceClip(VoiceClipEntity voiceClip)
    {
        ...
        // Outside the lock — see the note on VoiceClipLogged.
        if (notify) VoiceClipLogged?.Invoke();
        return persisted;
    }

    /// <summary>
    /// One-shot cleanup for databases written before the live path tokenised player names:
    /// rewrites every <c>voice_clips</c> row that carries the literal player name back into
    /// <c>-PlayerName-</c> / <c>-PlayerFirstName-</c> / <c>-PlayerLastName-</c>, and merges the
    /// row into its harvested twin when tokenising makes the two identical.
    ///
    /// Merging is safe for the audio: <c>VoiceMessageToFileName(RemovePlayerNameInText(text))</c>
    /// collapses both spellings to the same on-disk name, so the duplicate and the survivor
    /// always pointed at one and the same WAV. Nothing on disk is touched.
    ///
    /// <paramref name="dryRun"/> counts and samples without writing.
    /// </summary>
    public PlayerNameRetokenizeResult RetokenizePlayerNames(string playerName, bool dryRun,
        CancellationToken ct = default)
    {
        var result = new PlayerNameRetokenizeResult();
        ...
        lock (_writeLock)
        {
            ...
            foreach (var clip in CollectRetokenizeCandidates(playerName))
            {
                ct.ThrowIfCancellationRequested();
                RetokenizeOne(clip, playerName, dryRun, result);
            }

            if (!dryRun && result.HasWork)
            {
                _context.SaveChanges();
                notify = true;
            }

            // Drop the tracked graph either way: after an apply because the merge deleted rows
            // other cached reads may still hold, after a dry run because nothing it touched may
            // survive into the apply that follows.
            _context.ChangeTracker.Clear();
        }
    ...
    /// <summary>
    /// Every clip whose text mentions the player's first or last name. The full name always
    /// contains the first name, so these two terms cover all three tokens.
    /// </summary>
    private List<VoiceClipEntity> CollectRetokenizeCandidates(string playerName)
    {
        ...
        if (string.IsNullOrEmpty(first)) return new List<VoiceClipEntity>();

        var candidates = _context.VoiceClips
            .Where(vc => vc.OriginalText.Contains(first) || vc.CleanedText.Contains(first))
            .ToList();

        if (!string.IsNullOrEmpty(last))
        {
            var seen = candidates.Select(c => c.Id).ToHashSet();
            var byLast = _context.VoiceClips
                .Where(vc => vc.OriginalText.Contains(last) || vc.CleanedText.Contains(last))
                .ToList();
            candidates.AddRange(byLast.Where(c => seen.Add(c.Id)));
        }

        return candidates;
    }

    /// <summary>Tokenises one candidate: merge into its harvested twin if one exists, else
    /// rewrite in place. Counts into <paramref name="result"/> either way.</summary>
    private void RetokenizeOne(VoiceClipEntity clip, string playerName, bool dryRun,
        PlayerNameRetokenizeResult result)
    {
        const int maxSamples = 10;

        result.Scanned++;

        var newOriginal = PlayerNameTokenizer.Tokenize(clip.OriginalText, playerName);
        var newCleaned = PlayerNameTokenizer.Tokenize(clip.CleanedText, playerName);
        if (newOriginal == clip.OriginalText && newCleaned == clip.CleanedText) return;

        if (result.Samples.Count < maxSamples)
            result.Samples.Add($"{Truncate(clip.OriginalText)}  ->  {Truncate(newOriginal)}");

        // Does the tokenised text already exist for this character + base id? Then this row is
        // the live-path duplicate of a harvested row.
        var survivor = _context.VoiceClips.FirstOrDefault(vc =>
            vc.Id != clip.Id
            && vc.CharacterId == clip.CharacterId
            && vc.NpcBaseId == clip.NpcBaseId
            && vc.OriginalText == newOriginal);

        if (survivor != null)
        {
            result.MergedAndDeleted++;
            MergeGenerations(clip, survivor, dryRun, result);
            if (dryRun) return;

            // Keep whatever the survivor doesn't know yet: the live row is the one that learned
            // the on-disk state.
            if (string.IsNullOrEmpty(survivor.WavFileName) && !string.IsNullOrEmpty(clip.WavFileName))
                survivor.WavFileName = clip.WavFileName;
            if (!survivor.SavedToDisk && clip.SavedToDisk)
            {
                survivor.SavedToDisk = true;
                survivor.SavePath = clip.SavePath;
            }
            survivor.HasPlayerPlaceholder = TalkTextHelper.ContainsPlayerPlaceholder(survivor.OriginalText);

            _context.VoiceClips.Remove(clip);
            return;
        }
        ...
        clip.OriginalText = newOriginal;
        clip.CleanedText = newCleaned;
        clip.HasPlayerPlaceholder = TalkTextHelper.ContainsPlayerPlaceholder(newOriginal);
    }

    /// <summary>
    /// Re-points the duplicate's generation rows onto the survivor. A row that would collide on
    /// the unique <c>(voice_clip_id, player_content_id, alias_gender)</c> index is dropped
    /// instead — both rows describe the same file, since the filename hash is identical for the
    /// tokenised and the substituted spelling.
    /// </summary>
    private void MergeGenerations(VoiceClipEntity clip, VoiceClipEntity survivor, bool dryRun,
        PlayerNameRetokenizeResult result)
    {
        foreach (var gen in _context.VoiceClipGenerations.Where(g => g.VoiceClipId == clip.Id).ToList())
        {
            var collides = _context.VoiceClipGenerations.Any(g =>
                g.VoiceClipId == survivor.Id
                && g.PlayerContentId == gen.PlayerContentId
                && g.AliasGender == gen.AliasGender);

            if (collides)
            {
                result.GenerationsDropped++;
                if (!dryRun) _context.VoiceClipGenerations.Remove(gen);
            }
            else
            {
                result.GenerationsMoved++;
                if (!dryRun) gen.VoiceClipId = survivor.Id;
            }
        }
    }
    ...
    /// <summary>
    /// One page of harvested lines for a language, for the community-database bulk import.
    ///
    /// <para><b>Ordered by Id, not by timestamp.</b> The import walks ~288 pages and must survive
    /// being interrupted and resumed, so the order has to be stable across calls; a timestamp order
    /// would reshuffle as new lines arrive during the run and silently skip or repeat pages.</para>
    /// </summary>
    public List<VoiceClipEntity> GetVoiceClipsForLanguage(int language, int offset, int limit)
    {
        lock (_writeLock)
        {
    ...
    /// <summary>How many lines the import will have to walk for a language.</summary>
    public int GetVoiceClipCountForLanguage(int language)
    {
        lock (_writeLock)
        {
            if (_disposed) return 0;
            return _context.VoiceClips.AsNoTracking().Count(e => e.Language == language);
        }
    }
    ...
    public void WipeAll()
    {
        lock (_writeLock)
        {
            ...
            _cachedPhoneticModels = new List<PhoneticCorrection>();
    ...
    {
        var entities = _context.PhoneticCorrections
            .AsNoTracking()
            .ToList();
        _cachedPhonetics = entities;
        _cachedPhoneticModels = entities
            .Select(p => new PhoneticCorrection(p.OriginalText, p.CorrectedText))
            .ToList();
    }
    ...
    public List<AudioPackageExportRow> GetAudioPackageExportRows(int? language = null)
    {
        lock (_writeLock)
        {
            if (_disposed) return new List<AudioPackageExportRow>();

            var query =
                from g in _context.VoiceClipGenerations.AsNoTracking()
                join vc in _context.VoiceClips.AsNoTracking() on g.VoiceClipId equals vc.Id
                join c in _context.Characters.AsNoTracking() on vc.CharacterId equals c.Id
                where language == null || vc.Language == language.Value
                select new AudioPackageExportRow
                {
                    SavePath = g.SavePath,
                    AliasGender = g.AliasGender,
                    // The generation's voice is what was actually spoken; the clip's snapshot is
                    // the fallback for rows written before that column was filled.
                    VoiceKey = string.IsNullOrEmpty(g.VoiceKey) ? vc.VoiceKey : g.VoiceKey,
                    ...
                    NpcBaseId = vc.NpcBaseId,
                    TextSource = vc.TextSource,
                    QuestType = vc.QuestType,
                    BodyType = vc.BodyType,
                    OriginalText = vc.OriginalText,
                    CleanedText = vc.CleanedText,
                    HasPlayerPlaceholder = vc.HasPlayerPlaceholder,
                    WavFileName = vc.WavFileName,
                    ZoneName = vc.ZoneName,
                    MapX = vc.MapX,
                    MapY = vc.MapY,
                };
