...
public class NpcDataService : INpcDataService
{
    ...
    public void MigrateOldData(EchokrautVoice? oldVoice = null, EchokrautVoice? newEkVoice = null)
    {
        if (oldVoice == null)
        {
            ...
            if (oldPlayerMapData.Count > 0 || oldNpcMapData.Count > 0)
            {
                ...
                bool AdoptLegacyVoiceItem(NpcMapData entry)
                {
                    var matchedVoice = voices.FirstOrDefault(v => v.BackendVoice == entry.voiceItem?.Voice);
                    if (matchedVoice == null) return false;

                    entry.voice = matchedVoice.BackendVoice;
                    entry.voiceItem = null;
                    return true;
                }

                MigrateEntries(oldPlayerMapData, "player", AdoptLegacyVoiceItem);
                MigrateEntries(oldNpcMapData, "npc", AdoptLegacyVoiceItem);
            }
        }
        else
        {
            ...
            if (oldPlayerMapData.Count > 0 || oldNpcMapData.Count > 0)
            {
                ...
                bool AssignNewVoice(NpcMapData entry)
                {
                    entry.voice = newVoiceKey;
                    return true;
                }

                MigrateEntries(oldPlayerMapData, "player", AssignNewVoice);
                MigrateEntries(oldNpcMapData, "npc", AssignNewVoice);
            }
        }
    }

    /// <summary>
    /// Applies one migration step to a list of characters. Both branches of
    /// <see cref="MigrateOldData"/> ran this same loop four times over, differing only in the list,
    /// the word in the log line, and what the step does to the entry.
    /// <para><paramref name="apply"/> returns whether the entry actually changed and needs saving;
    /// the log line is written either way, matching the original behaviour (an unmatched legacy
    /// voice was still reported as "migrated").</para>
    /// </summary>
    private void MigrateEntries(List<NpcMapData> entries, string kind, Func<NpcMapData, bool> apply)
    {
        foreach (var entry in entries)
        {
            ...
            _log.Debug(nameof(MigrateOldData), $"Migrated {kind} {entry.Name}", new EKEventId(0, TextSource.None));
        }
    }
    ...
    /// <summary>
    /// Saves an edit in which the user corrected the character's gender/race. Those two are the
    /// lookup key and are re-read from the game on every line, so they are stored as OVERRIDES and
    /// the key is left exactly as the game reports it — otherwise the edited row becomes unreachable
    /// and the next spoken line simply recreates the original one (the reported duplicate).
    /// Pass <c>null</c> for either to drop that correction.
    /// </summary>
    public void SaveCharacterWithIdentityOverride(NpcMapData data, Genders? genderOverride, NpcRaces? raceOverride)
    {
        data.GenderOverride = genderOverride;
        data.RaceOverride = raceOverride;

        // Order matters: the upsert guarantees the row exists (and is keyed on the GAME identity)
        // before the override writer looks it up by that same identity.
        SaveCharacter(data);

        var entity = _db.FindCharacter(data.Name, data.Gender, data.Race, (int)data.Language);
        if (entity == null)
        {
            _log.Warning(nameof(SaveCharacterWithIdentityOverride),
                $"Character '{data.Name}' vanished between save and override write — correction not stored.",
                new EKEventId(0, TextSource.None));
            return;
        }

        _db.SetIdentityOverride(entity.Id, (int?)genderOverride, (int?)raceOverride);
    }
    ...
    public void SaveCharacter(NpcMapData data)
    {
        ...
        else if (!ReferenceEquals(canonical, data))
        {
            ...
            canonical.GenderOverride = data.GenderOverride;
            canonical.RaceOverride = data.RaceOverride;
    ...
    public void RemoveCharacterFromDatabase(NpcMapData data) => RemoveCharacterFromDb(data);
    ...
    private NpcMapData EntityToNpcMapData(CharacterEntity entity, string contextType)
    {
        var data = new NpcMapData((ObjectKind)entity.ObjectKind)
        {
            ...
            World = entity.World,
            GenderOverride = entity.GenderOverride.HasValue ? (Genders)entity.GenderOverride.Value : null,
            RaceOverride = entity.RaceOverride.HasValue ? (NpcRaces)entity.RaceOverride.Value : null,
        };
    ...
    private CharacterEntity NpcMapDataToEntity(NpcMapData data)
    {
        return new CharacterEntity
        {
            ...
            World = data.World ?? "",
            GenderOverride = data.GenderOverride.HasValue ? (int)data.GenderOverride.Value : null,
            RaceOverride = data.RaceOverride.HasValue ? (int)data.RaceOverride.Value : null,
        };
    }
