...
public class GameObjectService : IGameObjectService
{
    ...
    public void RefreshLocalPlayer()
    {
        ...
        // No player (title screen, logout, mid character switch). Clear rather than keep the
        // previous character's identity: a stale name would be substituted into another
        // character's dialog and baked into that character's audio files. An empty name is
        // handled defensively everywhere (placeholders stay unresolved, generation is refused),
        // a wrong one is not.
        _localPlayerName = "";
        _localPlayerContentId = 0;
    }

    private void CachePlayerIdentity(IGameObject player)
    {
    ...
    // Safe to read from any thread — refreshed on the framework tick via RefreshLocalPlayer()
    // and whenever LocalPlayer is accessed on the main thread.
