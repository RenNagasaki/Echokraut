...
public class AudioPlaybackService : IAudioPlaybackService, IDisposable
{
    ...
    // Concurrent, not a plain Dictionary: entries are added from the playback loop (via
    // PlayStream's onStreamCreated callback) and read/removed in OnSourceEnded, which BASS fires
    // on a thread-pool thread. A plain Dictionary can corrupt its buckets under that overlap.
    private readonly ConcurrentDictionary<Guid, VoiceMessage> _currentlyPlayingDictionary = new();
    ...
    public AudioPlaybackService(IVoiceMessageQueue queue, ILogService log, EKConfiguration configuration, IFramework framework, ILipSyncHelper lipSync, IAudioFileService audioFiles, ILiveGenerationLogger generationLogger)
    {
        ...
        // The configured audible range has to be applied HERE, not only from the settings slider.
        // Until this call existed, Update3DFactors ran exclusively from the three slider callbacks
        // in NativeConfigWindow, so a fresh session always used BASS's default rolloff of 1 while
        // the config said 0.3 — the setting only took effect once the user happened to touch the
        // slider, and was lost again on the next start (measured in-game 2026-08-03: BASS reported
        // rolloff 1 in a fresh session).
        Update3DFactors(_configuration.Voice3DAudibleRange);
    ...
    {
        // Doppler stays OFF (0). Live3DAudioEngine.ConfigureListener deliberately initialises it
        // that way, and this method used to pass 1 — so merely dragging the settings slider
        // switched Doppler on and pitch-shifted voices whenever the speaker or the player moved.
        // With the call above, that inconsistency would now hit every session at startup.
        Bass.Set3DFactors(1, audibleRange, 0);
        Bass.Apply3D();
    ...
    public void StopPlaying(VoiceMessage? message)
    {
        ...
        // Unconditional: a source that is still inside its (blocking) startup reports Stopped
        // although it is about to play. Gating the Stop on the state let exactly those through.
        _audioEngine.Stop(message.StreamId);
        // A stopped source never raises SourceEnded, so OnSourceEnded won't clean up after it —
        // drop the entry here or every skipped line leaks one.
        _currentlyPlayingDictionary.TryRemove(message.StreamId, out _);
    }
    ...
    private void OnSourceEnded(Guid guid)
    {
        ...
        // TryRemove, not TryGetValue: this is the end of the stream's life, so the entry goes with
        // it. Leaving it behind leaked one VoiceMessage per spoken line for the whole session.
        if (!_currentlyPlayingDictionary.TryRemove(guid, out var message))
        {
    ...
    private async Task PlayAudioAsync(VoiceMessageEntry entry, CancellationToken cancellationToken)
    {
        ...
        // Last line of defence: the source was cancelled after this message was built (the player
        // skipped past it), so it must not be heard even though it reached the play queue.
        if (_queue.IsObsolete(message))
        {
            _log.Info(nameof(PlayAudioAsync), "Skipping queued audio — line was skipped", eventId);
        ...
        try
        {
            ...
            // PlayStream blocks until the stream is buffered and running (seconds on a slow
            // streaming backend). The id is published through the callback so a cancellation
            // arriving inside that window has something to stop.
            message.StreamId = _audioEngine.PlayStream(
                ...
                {
                    message.StreamId = id;
                    _currentlyPlayingDictionary[id] = message;
                });

            // The line may have been skipped while the stream was starting up. The engine already
            // aborted the startup; make sure nothing downstream (lip sync, toolbar, completion)
            // treats it as if it had played.
            // (Deliberately not keyed on the engine state: a very short clip can legitimately have
            // finished already, and that path must keep its normal completion handling.)
            if (_queue.IsObsolete(message) || entry.State == Queue.VoiceMessageState.Cancelled)
            {
                _log.Info(nameof(PlayAudioAsync), "Discarding audio — line was skipped during stream startup", eventId);
                _currentlyPlayingDictionary.TryRemove(message.StreamId, out _);
                _audioEngine.Stop(message.StreamId);
                _queue.MarkAsCancelled(entry.Id);
                message.Stream?.Dispose();
