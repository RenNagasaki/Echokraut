...
namespace Echokraut.Backend
{
    public class AlltalkBackend : ITTSBackend
    {
        ...
        // onJobStarted is unused: AllTalk has no per-request handle, its stop endpoint is global.
        public async Task<Stream?> GenerateAudioStreamFromVoice(EKEventId eventId, VoiceMessage message, string voice, ClientLanguage language, Action<string>? onJobStarted = null)
        {
        ...
        public List<string>? GetAvailableVoices(EKEventId eventId)
        {
            ...
            {
                // Warning, not Error: an unreachable backend is the normal state at login and
                // every Error entry raises Dalamud's "Echokraut is creating errors" popup.
        ...
        // jobId is ignored — AllTalk's stop endpoint aborts whatever it is currently generating.
        public async Task StopGenerating(EKEventId eventId, string? jobId = null)
        {
            ...
            {
                _log.Warning(nameof(StopGenerating), ex.ToString(), eventId);
            }
        }
            ...
            try
            {
                ...
                if (!res.IsSuccessStatusCode)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(body))
                        msg += $" — {body.Trim()}";
                    _log.Warning(nameof(CheckReady), msg, eventId);
                    return msg;
                }
