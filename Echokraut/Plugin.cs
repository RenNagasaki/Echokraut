...
public partial class Plugin : IDalamudPlugin
{
        ...
        ITextureProvider textureProvider)
    {
        ...
        _gameObjects    = _services.GetService<IGameObjectService>();
        ...
        {
            // KTK's Initialize became InitializeAsync (it now loads its addon config file and
            // hops to the framework thread). We block on it deliberately: everything constructed
            // below — NativeWindowManager and every native window — assumes KTK is ready, and
            // fire-and-forget would race that. Blocking is safe ONLY because the plugin ctor runs
            // off the main thread (see the note further down about HandleStartup); on the framework
            // thread this would deadlock against the IFramework.Run inside InitializeAsync.
            KamiToolKit.KamiToolKitLibrary.InitializeAsync(_pluginInterface, $"Echokraut {PluginVersion}")
    ...
    private void WireEvents()
    {
        ...
        var dialogState = _services.GetService<IDialogStateService>();
    ...
    private void RunDataMigrationsIfLoggedIn()
    {
        ...
        try
        {
            // The audio backfill's placeholder detection reads LocalPlayerName; make sure the
            // cache is warm before it runs instead of relying on the framework tick having
            // happened to fire since login.
            _gameObjects.RefreshLocalPlayer();
            ...
            if (_configuration.AudioFilesBackfillPending)
            {
                ...
                db.BackfillAudioFiles(_configuration, _gameObjects, audioFiles);
            }
        }
        catch (Exception ex)
        {
    ...
    private void StartActiveEngineInstance()
    {
        ...
        // Off-thread: RefreshBackend blocks on the backend's voice-list request, and this runs
        // from HandleStartup, which is bounced onto the framework thread — blocking it here
        // stalls the first frames after login. The instance also fires OnInstanceReady →
        // RefreshBackend once it is actually up, so this is only the eager first attempt.
    ...
    /// <summary>Normal poll interval for the cached local-player identity (name / content id /
    /// gender). The values only change on a character switch, so once a minute is plenty.</summary>
    private const long PlayerRefreshIntervalMs = 60_000;

    /// <summary>Retry interval while the cache is cold (logged in but no name yet, e.g. right
    /// after login). Short, because everything that resolves a player placeholder silently
    /// degrades in that window — see <see cref="IGameObjectService.RefreshLocalPlayer"/>.</summary>
    private const long PlayerRefreshRetryMs = 2_000;

    private long _lastPlayerRefreshTick;

    private unsafe void OnFrameworkUpdate(Dalamud.Plugin.Services.IFramework fw)
    {
        RefreshLocalPlayerThrottled();
        ...
        if (_camera != null && localPlayer != null)
        {
            ...
            // The view matrix is row-major world→view, so its COLUMNS hold the camera axes in world
            // space: forward = column 2, up = column 1. Verified in-game against the game's own
            // CameraBase.CalculateSceneCameraYaw — this reading matches it to three decimals at
            // every camera angle (2026-08-03). The previous reading (m2,m1,m0 / m6,m5,m4) happens
            // to agree while the camera is level, which is why it looked correct, but drifts as
            // soon as it is pitched: measured 146° where the real yaw was 101°, with an "up" vector
            // pointing down.
    ...
    private void RefreshLocalPlayerThrottled()
    {
        var interval = string.IsNullOrEmpty(_gameObjects.LocalPlayerName)
            ? PlayerRefreshRetryMs
            : PlayerRefreshIntervalMs;

        var now = Environment.TickCount64;
        if (now - _lastPlayerRefreshTick < interval) return;

        _lastPlayerRefreshTick = now;
        _gameObjects.RefreshLocalPlayer();
    }
    ...
    public void Dispose()
    {
        ...
        if (_kamiToolKitInitialized)
        {
            // Cleanup was renamed to Dispose. The sync overload is the right one here: Dalamud
            // disposes plugins on the framework thread, which is what it requires — it refuses (and
            // leaks the nodes) when called from anywhere else. DisposeAsync is for callers that are
            // NOT on that thread, which a plugin Dispose is not.
            try { KamiToolKit.KamiToolKitLibrary.Dispose(); } catch { }
