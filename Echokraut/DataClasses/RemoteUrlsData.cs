...
public class RemoteUrlsData
{
    ...
    /// <summary>
    /// URL template for the wrapper's published engine list (<c>engines.json</c> in the wrapper repo
    /// root). The literal <c>{version}</c> is replaced with <see cref="EchokrauTtsVersion"/> so the
    /// list is read at the release tag users can actually install; the plugin then retries the
    /// wrapper's default branch and finally falls back to its own embedded copy. Empty disables the
    /// remote lookup (the embedded list is then the only source).
    /// <para>This exists so a wrapper release can add a sub-engine without a plugin release — see
    /// <see cref="TtsEngineCatalog"/>.</para>
    /// </summary>
    [JsonPropertyName("echokrauTtsEnginesUrl")]
    public string EchokrauTtsEnginesUrl { get; set; } = string.Empty;
    ...
    /// <summary>
    /// Base address of the Echolines community line database (no trailing slash); the client appends
    /// <c>/v1/lines</c> and <c>/v1/lines/import</c>. Lives here rather than as a constant so the
    /// service can move without a plugin release — every existing install would otherwise report
    /// into the void until it updates. <b>Empty disables reporting entirely</b>, which is also the
    /// kill switch if the service has to be taken down.
    /// </summary>
    [JsonPropertyName("echolinesUrl")]
    public string EcholinesUrl { get; set; } = string.Empty;
    ...
    /// <summary>
    /// Per-language voice packs, keyed like <see cref="VoiceNameUrls"/> (<c>German</c>,
    /// <c>English</c>, <c>French</c>, <c>Japanese</c>). The client's language decides which one is
    /// downloaded; a language with no entry falls back to English and then to
    /// <see cref="VoicePackUrl"/> — see <c>Helper/Functional/VoicePackUrlResolver</c>. Publishing a
    /// new language pack is one line here, no plugin release.
    /// </summary>
    [JsonPropertyName("voicePackUrls")]
    public Dictionary<string, string> VoicePackUrls { get; set; } = new();
}
