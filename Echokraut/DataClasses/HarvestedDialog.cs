...
/// <summary>
/// One unresolved speaker token from the scripted sheet families (<c>custom/*</c>,
/// <c>raid/*</c>, <c>warp/*</c>, …), written to
/// <c>&lt;localSaveLocation&gt;/harvest/scripted_alias_candidates.json</c>.
///
/// <para>These are lines we know are dialogue — the key carries a speaker-shaped token — but
/// whose token matches no NPC name and whose sheet has no single unambiguous owner. Measured
/// on live game data (2026-08-09) there are ~8,100 such lines in <c>custom/*</c> alone, led by
/// tokens like <c>PLATOONINSTRUCTOR</c> and <c>OMEGATERMINAL</c>. They are reported rather
/// than guessed at, because picking an NPC for them is a judgement call.</para>
///
/// <para><see cref="OwnerNpcIds"/> lists the ENpcBase rows that reference the sheet's
/// CustomTalk row. It is empty for families without a CustomTalk link, and holds more than one
/// entry when the sheet is shared — which is exactly the ambiguous case that stopped automatic
/// attribution.</para>
/// </summary>
public class ScriptedAliasCandidate
{
    public string Token { get; set; } = "";
    public string Sheet { get; set; } = "";
    public string SampleKey { get; set; } = "";
    public string SampleText { get; set; } = "";
    public List<uint> OwnerNpcIds { get; set; } = new();
