...
- **`EchokrauTtsData.TtsEngine`** (string, default **`"xtts"`**) = which sub-engine the LOCAL wrapper loads at startup, passed to the installer/wrapper as `--tts-backend` (normalized via `TtsBackendArg`). Only meaningful for Local (a Remote server's engine is fixed by its operator). The bootstrap installs ALL engines, so `IEchokrauTtsInstanceService.SwitchTtsBackend(string)` just **restarts** the running local instance (no reinstall). **Deliberately a string, not an enum**: the engine list comes from the wrapper repo at runtime (see `TtsEngineCatalog` below), so the config must be able to hold an engine this plugin build has never heard of. The old enum field `TtsBackend` (`EchokrauTtsEngine?`) survives **only** as the migration source — `Configuration.MigrateTtsEngineId()` folds it into `TtsEngine` and nulls it, so it cannot run twice. Dropdown lives in the shared `NativeEchokrauTtsBuilder.BuildLocalInstance` → appears in both Backend tab + First-Time wizard. **Threading the arg requires the installer** (`EchokrautLocalInstaller` `echokrautts` mode, positional `args[7]`, → `--tts-backend`), so it needs an installer re-release to take effect.
- **`TtsEngineCatalog` / `TtsEngineInfo` / `TtsEngineOption`** (`TtsEngineCatalog.cs`) = the wrapper's published engine list (`engines.json` in the **Echokrautts** repo root: `id`, `name`, `minVersion`). Fetched by `ITtsEngineCatalogService` from `RemoteUrlsData.EchokrauTtsEnginesUrl` — a template whose `{version}` is replaced by `EchokrauTtsVersion`, so the list is read **at the release tag users can install**, with the default branch as second try and `Resources/Engines.json` embedded as the offline fallback. `Helper/Functional/TtsEngineAvailability` turns the catalog + `InstalledWrapperVersion` into the dropdown entries: die Version, gegen die geprueft wird, liefert `GatingVersion`: **installierter Tag bei bestehender Installation, sonst der Tag, den eine Neuinstallation holen wuerde** (`LatestWrapperVersion`) — „keine Installation" heisst NICHT „alles verfuegbar", sonst bietet der Ersteinrichtungs-Assistent eine Engine an, die das dort installierte Release gar nicht enthaelt. Eine Engine ueber der Gating-Version ist **gelistet, aber nicht waehlbar**, beschriftet `"NAME (from wrapper version X)"`; im Ersteinrichtungs-Assistenten (`listUnavailable: false`) wird sie ganz ausgeblendet — ausser sie ist die konfigurierte, die bleibt sichtbar UND gesperrt (sonst faengt der Selbsterhalt-Zweig sie als „verfuegbar" wieder ein). Das Dropdown zieht im Per-Frame-`Update` nach (`RefreshEngineOptions`, nur bei geaenderter Signatur und zugeklapptem Popup), weil die Nodes in `OnSetup` entstehen und ein Fenster-Neuoeffnen sie nicht neu baut. **`minVersion` gating is the one place wrapper tags are compared numerically** (`WrapperUpdatePolicy.IsAtLeast`); the update offer keeps its ordinal string compare. Anything unparseable on either side answers *available* — wrongly hiding a working engine is the more expensive mistake.
...
- **`EchokrauTtsData.InstalledWrapperVersion`** (string, default `""`) = git release tag of the wrapper currently extracted under `{TtsInstallRoot}\echokrautts`. Written by `IEchokrauTtsInstanceService` after a successful install/update from `RemoteUrlsData.EchokrauTtsVersion`; compared against that same remote value by `Helper/Functional/WrapperUpdatePolicy` to show the Backend-tab Update button + version label. Installs predating the handshake carry no value and are backfilled once by `MigrateWrapperVersionForExistingInstalls()` (run from `Initialize`) with `WrapperUpdatePolicy.AssumedLegacyVersion` = the single published wrapper release — otherwise they'd show "?" and be offered an update that re-downloads what they already run. **Do not bump that constant when a newer wrapper ships**: existing installs are then genuinely behind and must get the update (`WrapperVersionMigrationTests.AssumedLegacyVersion_MatchesTheWrapperReleaseWeShip` goes red at that point as the reminder to delete the assertion, not to raise the constant). The migration is gated on `LocalInstall` — Remote/None users keep the empty value. See `Services/CLAUDE.md` (wrapper update handshake) for the installer side.
...
## Install root default (`C:\LocalTTS`, moved 2026-09-07)

`Configuration.DefaultTtsInstallRoot` is `C:\LocalTTS`; the previous default `C:lltalk_tts`
survives as `LegacyDefaultTtsInstallRoot`. Only the **root** moved — the per-engine subfolders
(`alltalk_tts\`, `echokrautts\`, see `TtsPaths`) are unchanged.

- **Never compare against `DefaultTtsInstallRoot` alone** to answer "is this still the shipped
  default?" — use `IsDefaultTtsInstallRoot`, which accepts both. `MigrateTtsInstallRoot` asks that
  question twice, and getting it wrong breaks in both directions: an existing install sitting on the
  old root reads as a deliberate custom root, and — the sharp edge — `AlltalkData.LocalInstallPath`
  **still initialises to the old path**, so on a fresh install the untouched initialiser reads as a
  "custom" legacy root and drags every new user back onto `C:lltalk_tts`.
  `Migrate_FreshInstall_IsNotDraggedOntoTheLegacyRoot` guards exactly that (verified to fail when the
  legacy arm is removed).
- `LocalSaveLocation` is derived from `DefaultTtsInstallRoot` now; it used to hard-code the old root
  a second time and would have been left pointing at an unused folder.
- `DialogHarvestService.HarvestBaseDir` replaced five copies of
  `SaveToLocal ? LocalSaveLocation : @"C:lltalk_tts"` and follows `TtsInstallRoot` instead — right
  for an existing install, a fresh one, and a user who moved their root.
- Existing users are never relocated: their root is persisted, so the new default only reaches fresh
  configs.
