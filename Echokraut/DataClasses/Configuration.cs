...
public class Configuration : IPluginConfiguration
{
    ...
    internal const string DefaultTtsInstallRoot = "C:\\LocalTTS";

    /// <summary>
    /// The default install root shipped before <see cref="DefaultTtsInstallRoot"/> moved to
    /// <c>C:\LocalTTS</c>. Kept because "is this value still the untouched default?" is the
    /// question <see cref="MigrateTtsInstallRoot"/> asks, and after the move there are TWO answers:
    /// a config written by an older version carries the old string and must not be mistaken for a
    /// deliberate choice. Dropping this constant would also break fresh installs outright —
    /// <see cref="AlltalkData.LocalInstallPath"/> still initialises to the old path, so the
    /// migration would read it as a "custom" legacy root and silently pull every NEW user back to
    /// <c>C:\alltalk_tts</c>.
    /// <para>Not for building paths — only for recognising defaults.</para>
    /// </summary>
    internal const string LegacyDefaultTtsInstallRoot = "C:\\alltalk_tts";

    /// <summary>True for any value that is a shipped default rather than a user's own choice.</summary>
    internal static bool IsDefaultTtsInstallRoot(string? path)
        => path == DefaultTtsInstallRoot || path == LegacyDefaultTtsInstallRoot;

    /// <summary>
    /// Report every unvoiced dialogue line to the Echolines community database. <b>Default on</b>,
    /// and deliberately so: the database only becomes useful once enough installations contribute.
    /// <para>Offered in the first-time wizard AND under Settings → General — one value, two places,
    /// because existing users never see the wizard again. For them the changelog entry is the
    /// disclosure, not documentation.</para>
    /// <para>What is sent: the dialogue line (with the character name already replaced by a
    /// placeholder), its language, and which NPC speaks it. Nothing about the player.</para>
    /// </summary>
    public bool ShareLinesWithCommunityDb { get; set; } = true;

    /// <summary>
    /// Random per-installation GUID, derived from nothing. Sole purpose: the server counts distinct
    /// installations for its consensus rule and partitions its rate limit. It is NOT an account, not
    /// derived from the character or the machine, and never leaves this config except as this value.
    /// <para>Generated on first use rather than in the initialiser so an existing config does not
    /// silently gain one before the user has seen the switch.</para>
    /// </summary>
    public string EcholinesInstallId { get; set; } = "";

    /// <summary>Per-language resume state for the harvest bulk import: language code → next chunk
    /// index to send. A harvest runs per language and is ~288 chunks, so an aborted run has to
    /// continue where it stopped rather than start over.</summary>
    public Dictionary<string, int> EcholinesImportProgress { get; set; } = new();
    ...
    public string LocalSaveLocation { get; set; } = DefaultTtsInstallRoot + @"\LocalSaves";
    ...
    public void Initialize(IDalamudPluginInterface pluginInterface)
    {
        ...
        MigrateWrapperVersionForExistingInstalls();
        MigrateTtsEngineId();
        MigrateLegacyLogConfig();
    }

    /// <summary>
    /// One-shot backfill of <see cref="EchokrauTtsData.InstalledWrapperVersion"/> for local installs
    /// made before the wrapper version handshake existed. Their wrapper can only be
    /// <see cref="WrapperUpdatePolicy.AssumedLegacyVersion"/> — the single published release — so
    /// recording it keeps the Backend tab from showing "?" and from offering an update that would
    /// re-download the build the user already runs.
    /// <para>Idempotent: gated on a blank value, and only for a completed local install. Remote/None
    /// users have no wrapper on disk, so their empty tag stays empty and gets the real value from
    /// the install that eventually writes it.</para>
    /// </summary>
    public void MigrateWrapperVersionForExistingInstalls()
    {
        if (EchokrauTts is not { LocalInstall: true } ek) return;
        if (!string.IsNullOrWhiteSpace(ek.InstalledWrapperVersion)) return;

        ek.InstalledWrapperVersion = WrapperUpdatePolicy.AssumedLegacyVersion;
    }

    /// <summary>
    /// One-shot move of the EchokrauTTS sub-engine from the old <c>EchokrauTtsEngine</c> enum to the
    /// wrapper's own engine id. The enum could only ever hold the two engines the plugin was compiled
    /// with; the id comes from the wrapper's published catalog and survives engines the plugin does
    /// not know. Idempotent: the legacy field is cleared, so a config written after the migration has
    /// nothing left to migrate.
    /// </summary>
    public void MigrateTtsEngineId()
    {
        if (EchokrauTts?.TtsBackend is not { } legacy) return;

        EchokrauTts.TtsEngine = legacy.ToString().ToLowerInvariant();
        EchokrauTts.TtsBackend = null;
    }
    ...
    /// is still at a shipped default and the legacy path holds a meaningful custom value. Idempotent —
    /// once a user has a non-default root it is never overwritten, and an empty/blank legacy path is
    /// ignored (the default root stands, which also fixes old configs that saved an empty path).
    /// <para><b>"Default" means either shipped default</b> (<see cref="IsDefaultTtsInstallRoot"/>),
    /// not just the current one. Comparing against the current default alone would treat an existing
    /// install sitting on the old path as a deliberate custom root on one side, and the untouched
    /// <see cref="AlltalkData.LocalInstallPath"/> initialiser as a custom legacy root on the other —
    /// the latter would move every fresh install back onto the old path.</para>
    /// </summary>
    public void MigrateTtsInstallRoot()
    {
        if (!string.IsNullOrWhiteSpace(TtsInstallRoot) && !IsDefaultTtsInstallRoot(TtsInstallRoot))
            return; // user already on a custom root — leave it.

        var legacy = Alltalk?.LocalInstallPath;
        if (!string.IsNullOrWhiteSpace(legacy) && !IsDefaultTtsInstallRoot(legacy))
