...
| `NativeGameDataToolsWindow` | `NativeGameDataToolsWindow.cs` | Data-pipeline window: Quest Dialog Harvest, Voice Pack download, NPC Attribution Repair, **Player Name Cleanup** (`IDatabaseService.RetokenizePlayerNames`), **Import / Export** of `*.ekpack` audio packages (`IAudioPackageService` — path input + language dropdown + Export/Import buttons, both driven by the window's shared progress bar; buttons go through `Dim` since one service serialises both directions), Quest Lua Debug. The two DB-cleanup sections share one shape — Dry Run first, Apply enabled only after a dry run reported work, both run in `Task.Run` with their own `CancellationTokenSource`, output text written under `_statusLock` and drained into the `TextNode` in `OnUpdate`. |
...
- **The detail window shows the STORED text, placeholders included** — it must not substitute the
  local player's name into `-PlayerFirstName-` & co. The row is player-independent, and the
  substitution silently no-ops while the player-name cache is cold, so the same row used to read
  differently before and after the session's first dialog (which looked like the DB row had been
  overwritten). See `Services/CLAUDE.md` → "Player-name placeholders".
...
## KamiToolKit version + which shared workarounds still apply (checked 2026-09-07)

Pinned at `6b7b191` ("Fix empty slots being interactable"), updated from `f5f2970` (42 commits).
The parent `../../CLAUDE.md` lists KTK workarounds shared by all plugins; those were written against
the older pin, so this is what re-checking them against `6b7b191` found. **Re-check this list on the
next KTK bump** — a workaround kept past its cause is worse than none, because it hides the real API.

- **`TextDropDownNode` crash workaround — DEAD.** The type no longer exists in KTK; the plugin has
  been on `StringDropDownNode` for a while. Nothing to work around.
- **`TabBarNode.SelectTab` only highlights — STILL TRUE.** It now routes through `ClickHandler`
  (`Nodes/TabBarNode.cs:47-52`), but that only flips `IsChecked`/`IsSelected` on the radio buttons
  (`:183-191`). No per-tab callback runs, so still call the show method yourself.
- **`DropDownNode` always opens scrolled to the top — CONFIRMED IN SOURCE, and NOT fixable from a
  plugin.** `Uncollapse()` sets `scrollPosition = 0` and `PopupScrollbarNode.ScrollPosition = 0`
  (`Nodes/ComponentNode/DropDownNode.cs:199-201`) regardless of `SelectedOption`. Both the field and
  `PopupScrollbarNode` (`:501`) are `private`, as is `PopulatePopupList` — there is no public seam to
  scroll to the selection afterwards. This is the reported "voice list always starts from the top".
  A plugin-side mitigation is to order the current selection first; the real fix belongs upstream.
- **`TreeListCategoryNode` collision inside `ScrollingListNode` — NOT re-checked.** Runtime
  behaviour, not decidable by reading; still assume the documented workaround holds.

### Init / dispose contract (changed in this bump)
`KamiToolKitLibrary.Initialize`/`Cleanup` are gone: `InitializeAsync` and `Dispose`/`DisposeAsync`.
See `Plugin.cs` for why init is awaited synchronously (everything built afterwards assumes KTK is
ready, and the ctor is off the framework thread) and why dispose uses the **sync** overload (it
requires the framework thread and refuses otherwise, which is exactly where Dalamud disposes plugins).
