...
public sealed unsafe class NativeVoiceClipManagerWindow : NativeAddon
{
    ...
    protected override void OnSetup(AtkUnitBase* addon, Span<AtkValue> atkValueSpan)
    {
        ...
        // Game Data Tools button — opens the bulk-data window (harvest + voice pack).
        ...
        for (var i = 0; i < 2; i++)
        {
            ...
            {
                if (row == null) return;
                var mapData = _npcData.MappedNpcs.FirstOrDefault(n => n.ToString() == row.NpcKey)
                           ?? _npcData.MappedPlayers.FirstOrDefault(n => n.ToString() == row.NpcKey);
    ...
    protected override void OnUpdate(AtkUnitBase* addon)
    {
        ...
        // Game Data Tools icon button: ApplyEnabled routes through ATK's component-disabled state
        // (SetEnabledState triggers the FFXIV-standard disabled timeline — alpha ~0.7, multiplier
        // 0.5 — and silences cursor flip + click anim) AND strips the ImageNode's mouse flags, which
        // is where this button's click actually arrives. The hover handlers wired on ImageNode bail
        // on IsEnabled (see OnSetup); the disable transition itself force-clears any stuck hover
        ...
        if (_gameDataToolsButton != null && _gameDataBtnEnabledState != liveGen)
        {
            ...
            _gameDataToolsButton.ApplyEnabled(liveGen);
            if (!liveGen)
            {
    ...
    private void StartProgressive()
    {
        ...
        _npcList = _npcData.MappedNpcs
            .Where(n => n.Language == _clientState.ClientLanguage
    ...
    // Bulk "Generate All Unsaved" — fully disabled in None mode (no backend to call), so the click
    // is swallowed by ATK instead of running through the no-op generation gate. NodeState does the
    // dim + disable together and only writes on a change. Per-clip buttons live in the detail window.
    private void UpdateGenAllButtonState(bool liveGen)
        => NodeState.SetEnabled(_genAllToggleButton, liveGen);
