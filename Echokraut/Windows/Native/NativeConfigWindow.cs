    ...
    private ScrollingListNode BuildGeneralPanel(Vector2 pos, Vector2 size)
    {
        ...
        _clearNpcsButton = Button(Loc.S("Clear mapped NPCs"), 160, () =>
        {
            if (_deleteNpcsArmed)
            {
                ...
                foreach (var npc in _npcData.MappedNpcs.Where(p => !p.Name.StartsWith("BB")).ToList())
                {
                    _audioFiles.RemoveSavedNpcFiles(_config.LocalSaveLocation, npc.Name);
                    _npcData.RemoveCharacter(npc);   // drops the cache entry as well
                }
            }
            else
            {
        ...
        _clearPlayersButton = Button(Loc.S("Clear mapped players"), 160, () =>
        {
            if (_deletePlayersArmed)
            {
                ...
                // deleted exactly one player and then blew up. (The NPC and bubble buttons above
                // and below already iterate a FindAll copy, which is why only this one broke.)
                foreach (var p in _npcData.MappedPlayers.ToList())
                {
                    _audioFiles.RemoveSavedNpcFiles(_config.LocalSaveLocation, p.Name);
                    _npcData.RemoveCharacter(p);   // drops the cache entry as well
                }
            }
            else
            {
