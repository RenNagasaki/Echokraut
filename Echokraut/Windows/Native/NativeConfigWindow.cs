...
public sealed unsafe partial class NativeConfigWindow : NativeAddon
{
    ...
    // Bulk clears run off the frame thread — see RunBulkClear. One shared guard so the three
    // buttons can't overlap each other (they all delete characters through the same DbContext).
    private volatile bool _clearRunning;
    private volatile bool _clearCompleted;
    private int _clearedCount;
    private TextButtonNode? _clearButtonInProgress;
    ...
    protected override void OnUpdate(AtkUnitBase* addon)
    {
        ...
        if (_clearCompleted)
        {
            ...
            _clearRunning = false;
            try { _npcData.ClearMappedCaches(); } catch { }
            if (_clearButtonInProgress != null)
                _clearButtonInProgress.String = _clearButtonLabel;
            _clearButtonInProgress = null;
            _log.Info(nameof(OnUpdate), $"Cleared {_clearedCount} mapped characters.",
                new EKEventId(0, TextSource.None));
        }
        ...
        // Deferred voice remap after a successful connection test (see TestConnection). Off the
        // frame thread: RefreshBackend → MapVoices → GetAvailableVoices is a *blocking* HTTP call
        // (5s timeout), which froze the game for that long whenever the backend went away between
        // the test and this remap. Safe to run in the background — the VoicesMapped/CharacterMapped
        // handlers only set rebuild flags, and the actual node work happens in this OnUpdate.
        if (_pendingRemapAfterConnect)
        {
            _pendingRemapAfterConnect = false;
            Task.Run(() => _backend.RefreshBackend());
        }
        ...
        // Batch lock — harvest / voice pack download / (future) import / export. While any
        ...
        // Mode switcher — the SELECTED button is the dimmed one: dimming now really disables
        // (NodeState), and the mode you are already in is the one with nothing left to do.
        // Highlighting it the other way round would disable exactly the buttons you switch with.
        // All three lock while an install is in progress (a mid-install switch would race on
        // alltalkFolder) AND while a batch op (harvest / voice-pack download) runs — switching
        // modes would change which backend the op talks to mid-flight.
        ...
        // audio source mid-batch, but the voice pack download writes under the same local
        // save root and we don't want it pointed at a moving target.
    ...
    /// <summary>
    /// Deletes a set of mapped characters off the frame thread.
    ///
    /// <para>⚠ These three buttons used to do the whole job inside the ATK click handler, i.e. on
    /// the game's main thread: per character a recursive <c>Directory.Delete</c>, a
    /// <c>FindCharacter</c> query and a <c>DeleteCharacter</c> — and the latter runs
    /// <c>SaveChanges()</c> <b>plus a full <c>RefreshCharacterCaches()</c></b> whenever
    /// <c>BulkMode</c> is off. That is N cache rebuilds, N commits and N recursive directory
    /// deletes without ever yielding a frame. It went unnoticed while N was "NPCs you actually
    /// met"; once the dialog harvest fills the database with tens of thousands of characters it
    /// freezes and then kills the game.</para>
    ///
    /// <para>Two fixes, both needed: run it in a <see cref="Task"/> (so the frame thread keeps
    /// running) and hold <c>BulkMode</c> for the duration, which collapses the N cache rebuilds
    /// into the single <c>RefreshCaches</c> in the <c>finally</c>. The snapshot is taken on the
    /// calling thread because the mapped lists are the UI's own caches.</para>
    /// </summary>
    private void RunBulkClear(TextButtonNode button, string idleLabel, Func<List<NpcMapData>> snapshot)
    {
        List<NpcMapData> targets;
        try { targets = snapshot(); }
        catch (Exception ex)
        {
            _log.Error(nameof(RunBulkClear), $"Could not read the character list: {ex}", new EKEventId(0, TextSource.None));
            button.String = idleLabel;
            return;
        }

        if (targets.Count == 0) { button.String = idleLabel; return; }

        _clearRunning = true;
        _clearButtonInProgress = button;
        _clearButtonLabel = idleLabel;
        button.String = Loc.S("Clearing...");
        ...
        Task.Run(() =>
        {
            ...
            try
            {
                ...
                foreach (var entry in targets)
                {
                    try
                    {
                        _audioFiles.RemoveSavedNpcFiles(_config.LocalSaveLocation, entry.Name);
                        // DB-only: the mapped lists are plain List<> that OnUpdate enumerates, so
                        // they must not be mutated from here. They are rebuilt in one go by the
                        // ClearMappedCaches call on the frame thread once this finishes.
                        _npcData.RemoveCharacterFromDatabase(entry);
                        removed++;
                    }
                    catch (Exception ex)
                    {
                        // One bad row (locked folder, already-deleted character) must not abort
                        // the whole clear — the user asked for all of them to go.
                        _log.Warning(nameof(RunBulkClear), $"Could not remove '{entry.Name}': {ex.Message}",
                            new EKEventId(0, TextSource.None));
                    }
                }
            }
            ...
            {
                _log.Error(nameof(RunBulkClear), $"Clear failed: {ex}", new EKEventId(0, TextSource.None));
            }
            finally
            {
                try
                ...
                {
                    _log.Warning(nameof(RunBulkClear), $"Cache refresh after clear failed: {ex.Message}",
                        new EKEventId(0, TextSource.None));
                }
                _clearedCount = removed;
    ...
    private ScrollingListNode BuildGeneralPanel(Vector2 pos, Vector2 size)
    {
        ...
        // Same value as the first-time wizard offers. It lives here too because existing users
        // never see that wizard again, and a setting nobody can find is not a choice.
        var shareLinesCheck = Check(
            Loc.S("Help complete the community dialogue database"), w,
            _config.ShareLinesWithCommunityDb,
            v => { _config.ShareLinesWithCommunityDb = v; _config.Save(); });
        var shareLinesHint = Label(
            Loc.S("Sends the dialogue line, its language and the NPC. Never your character name."), w);
        ...
        {
            if (_clearRunning) return;
            if (_deleteNpcsArmed)
            {
                _deleteNpcsArmed = false;
                RunBulkClear(_clearNpcsButton!, Loc.S("Clear mapped NPCs"),
                    () => _npcData.MappedNpcs.Where(p => !p.Name.StartsWith("BB")).ToList());
            }
            else
            {
        ...
        _clearPlayersButton = Button(Loc.S("Clear mapped players"), 160, () =>
        {
            ...
            {
                _deletePlayersArmed = false;
                // ToList() first: the loop removes from the very list it walks. Iterating the
                // live list threw "Collection was modified" on the first removal, so this button
                // deleted exactly one player and then blew up.
                RunBulkClear(_clearPlayersButton!, Loc.S("Clear mapped players"),
                    () => _npcData.MappedPlayers.ToList());
            }
            else
            {
                _lastDeleteClick = DateTime.Now;
        ...
        _clearBubblesButton = Button(Loc.S("Clear mapped bubbles"), 160, () =>
        {
            ...
            {
                _deleteBubblesArmed = false;
                RunBulkClear(_clearBubblesButton!, Loc.S("Clear mapped bubbles"),
                    () => _npcData.MappedNpcs.Where(p => p.Name.StartsWith("BB")).ToList());
            }
            else
            {
        ...
        // Off-thread: SetBackendType re-pulls the voice list over HTTP and blocks until the
        // request finishes or times out — on the click (frame) thread that is a visible freeze.
        _atReloadVoicesButton = Button(Loc.S("Reload voices"), 180, () => Task.Run(() =>
        {
        ...
        list.AddNode(shareLinesCheck);
        list.AddNode(shareLinesHint);
    ...
    /// <summary>
    /// Build a mode-switcher button styled to look like a small radio. <b>The selected mode is the
    /// dimmed one</b> — dimming disables (see <c>NodeState</c>), and the mode already active has
    /// nothing left to do, while the others stay bright and clickable. Applied per frame in
    /// OnUpdate since InstanceType is the source of truth and may flip from the FTU window or
    /// another code path; `NodeState` only writes on an actual change, so that costs nothing.
    ///
    ...
    private void ProcessBackendSelection(string option)
    {
        if (Enum.TryParse<TTSBackends>(option, out var backend))
        {
            ...
            // reconnects the backend. No-op when already on that engine. Off-thread because the
            // reconnect blocks on the new engine's voice-list request (and the voice copy is file
            // I/O) — the dropdown updates below stay on the frame thread, where ATK needs them.
            Task.Run(() => _voiceSync.SwitchEngine(backend));
        }
