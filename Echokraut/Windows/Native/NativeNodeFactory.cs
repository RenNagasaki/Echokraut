...
internal static class NativeNodeFactory
{
    ...
    /// <summary>
    /// Enabled/disabled state of a node: dims it <b>and</b> makes it actually unusable.
    /// Null-safe and idempotent.
    /// <para>Thin alias for <see cref="NodeState.SetEnabled"/> in Echotools.UI, which is where the
    /// rule lives so every plugin gets it. Kept under the old name because ~76 call sites read
    /// <c>Dim(x, enabled)</c> and the shorthand is the established vocabulary in this layer. See
    /// <see cref="NodeState"/> for why alpha alone does not disable anything.</para>
    /// </summary>
    public static void Dim(NodeBase? node, bool enabled) => NodeState.SetEnabled(node, enabled);

    /// <summary>Null-safe visibility toggle. Idempotent, same rationale as <see cref="Dim"/>.</summary>
    public static void SetVisible(NodeBase? node, bool visible)
    {
        if (node != null && node.IsVisible != visible) node.IsVisible = visible;
    }
