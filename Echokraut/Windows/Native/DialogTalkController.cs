...
public sealed unsafe class DialogTalkController : IDisposable
{
    ...
    // The voices currently offered by the dropdown, positionally aligned with its Options.
    // The picked label is resolved against THIS snapshot (see VoiceOptionList) instead of a
    // name lookup in a freshly fetched list — display names are not guaranteed unique.
    private List<EchokrautVoice> _dropDownVoices = new();
    ...
    public void SetWindowHitTest(Func<Vector2, bool> hitTest)
    {
        ...
        _dialogState.IsInsideOwnedWindow = hitTest;
    }
    ...
    private void DisposeNodes()
    {
        ...
        _dropDownVoices = new List<EchokrautVoice>();
    ...
    private void OnUpdateInner(AtkUnitBase* addon)
    {
        ...
        var msg = _dialogState.CurrentVoiceMessage;
        ...
        var notVoiced     = !_dialogState.IsVoiced;
        var isMuted       = IsMuted();
        ...
        if (voiceUsable)
        {
            ...
            if (speakerKey != _lastSpeakerKey)
            {
                ...
                _dropDownVoices   = voices;
                var voiceNames    = VoiceOptionList.BuildLabels(voices);
                var selectedVoice = VoiceOptionList.LabelFor(voices, speaker.Voice);
    ...
    private void ProcessVoiceSelection(string voiceName)
    {
        ...
        if (msg?.Speaker != null)
        {
            // Resolve against the exact list the dropdown was built from — a name lookup in a
            // freshly fetched list picks the wrong entry when two samples share a display name.
            var newVoice = VoiceOptionList.Resolve(_dropDownVoices, voiceName);

            if (newVoice != null &&
                !string.Equals(newVoice.BackendVoice, msg.Speaker.voice, StringComparison.OrdinalIgnoreCase))
            {
    ...
    /// <summary>
    /// Enabled state of a toolbar control. Goes through <see cref="NodeState"/> so a dimmed control
    /// is really unusable — this used to set the alpha only, which ATK ignores for input.
    /// </summary>
    private static void SetEnabled(NodeBase node, bool enabled)
        => NodeState.SetEnabled(node, enabled, DisabledAlpha);
    ...
    private bool IsMuted()
        ...
        => _dialogState.CurrentVoiceMessage?.SpeakerObj != null
           && _npcData.IsDialogueMuted(_dialogState.CurrentVoiceMessage.SpeakerObj.BaseId);
    ...
    {
        if (_dialogState.CurrentVoiceMessage?.SpeakerObj == null) return;

        if (isChecked)
        {
            _npcData.MuteDialogue(_dialogState.CurrentVoiceMessage.SpeakerObj.BaseId);
            if (_audioPlayback.IsPlaying)
            {
                _lipSync.TryStopLipSync(_dialogState.CurrentVoiceMessage);
                _audioPlayback.StopPlaying(_dialogState.CurrentVoiceMessage);
            }
        }
        else
        {
            _npcData.UnmuteDialogue(_dialogState.CurrentVoiceMessage.SpeakerObj.BaseId);
            _recreateInference();
        }
    }
