...
public static class NativeAlltalkBuilder
{
    ...
    public static (bool isValid, string message) ValidateInstallPath(string? path)
    {
        ...
        if (!System.IO.Path.IsPathRooted(path))
            return (false, Loc.S("Please enter an absolute path (e.g. C:\\LocalTTS)."));
        ...
        if (string.IsNullOrEmpty(parent))
            return (false, Loc.S("The path must point to a subfolder, not the root of a drive (e.g. C:\\LocalTTS)."));
