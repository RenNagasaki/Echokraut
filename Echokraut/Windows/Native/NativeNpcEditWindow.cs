...
public sealed unsafe class NativeNpcEditWindow : NativeAddon
{
        ...
        Action onSaved)
    {
        ...
        // Effective*, so reopening the window shows the correction the user made last time rather
        // than snapping back to what the game reports.
        _pendingRace = _npc.EffectiveRace;
        _pendingGender = _npc.EffectiveGender;
    ...
    protected override void OnSetup(AtkUnitBase* addon, Span<AtkValue> atkValueSpan)
    {
        ...
        var currentRaceName = _npc.EffectiveRace.ToString();
        ...
        var currentGenderName = _npc.EffectiveGender.ToString();
        ...
        _selectableVoices = allVoices
            .FindAll(v => v.IsSelectable(_npc.Name, _npc.EffectiveGender, _npc.EffectiveRace, _npc.BodyType));
    ...
    /// <summary>Log form of an optional override: "none" reads better than an empty string.</summary>
    private static string Describe<T>(T? value) where T : struct => value.HasValue ? value.Value.ToString()! : "none";

    private void SaveClicked()
    {
        try
        {
            // A correction is stored only where it actually differs from what the GAME reports;
            // picking the game's own value back means "no correction", not "correction to the same
            // thing", so the override is dropped and the entry stops being marked as edited.
            var genderOverride = _pendingGender != _npc.Gender ? _pendingGender : (Genders?)null;
            var raceOverride = _pendingRace != _npc.Race ? _pendingRace : (NpcRaces?)null;

            var overrideChanged = genderOverride != _npc.GenderOverride || raceOverride != _npc.RaceOverride;
            var anyChange = overrideChanged
                || _pendingVoiceKey != (_npc.voice ?? "")
            ...
            // Name, Gender and Race are NOT touched: they are the lookup key and are re-derived from
            // the game on every line. Writing the user's opinion into them orphans the row and the
            // next spoken line recreates the original — the duplicate this window used to produce.
            _npc.voice = _pendingVoiceKey;
            _npc.Volume = _pendingVolume;
            _npc.IsEnabled = _pendingEnabled;

            _npcData.SaveCharacterWithIdentityOverride(_npc, genderOverride, raceOverride);

            _log.Info(nameof(NativeNpcEditWindow),
                $"Updated character {_npc.Name} (game: {_npc.Gender}/{_npc.Race}): " +
                $"GenderOverride={Describe(genderOverride)}, RaceOverride={Describe(raceOverride)}, " +
                $"Voice='{_pendingVoiceKey}', Volume={_pendingVolume:0.00}, Enabled={_pendingEnabled}",
