...
public sealed unsafe partial class NativeConfigWindow
{
    ...
    private void UpdateVoiceSelection()
    {
        ...
        if (_vsNpcToRemove != null)
        {
            ...
            // RemoveCharacter drops the cache entry too, so the per-list dispatch that used to
            // sit here is gone — it duplicated the service's own ObjectKind rule.
    ...
    private void StartMappedBuild(ScrollingListNode panel, IReadOnlyList<NpcMapData> source, bool isBubble, int tabIndex)
    {
