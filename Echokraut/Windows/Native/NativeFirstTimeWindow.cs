...
public sealed unsafe class NativeFirstTimeWindow : NativeAddon
{
    ...
    private CheckboxNode? _shareLinesCheck;
    private TextNode? _shareLinesHint;
    ...
    protected override void OnSetup(AtkUnitBase* addon, Span<AtkValue> atkValueSpan)
    {
        ...
        // Wizard: only the engines the release being installed actually contains. Nothing here can
        // unlock a newer one, so a blocked entry would be dead weight (Backend tab shows them).
        _ekLocalNodes = NativeEchokrauTtsBuilder.BuildLocalInstance(w, _config, _echokrauTtsInstance,
            _alltalkInstance.IsCudaInstalled, listUnavailableEngines: false);
        ...
        // Offered on the last page, where the user is not busy deciding anything else. Default on,
        // and said plainly rather than buried: the sentence below the box is the consent, so it names
        // what is sent and what is not instead of only praising the feature.
        _shareLinesCheck = Chk(Loc.S("Help complete the community dialogue database"), w,
        ...
        list.AddNode(_shareLinesCheck);
        list.AddNode(_shareLinesHint);
    ...
    protected override void OnUpdate(AtkUnitBase* addon)
    {
        ...
        // The SELECTED engine is the dimmed one — dimming really disables (NodeState), so marking
        // the active engine bright would disable exactly the button you switch with. Both lock
        // during a batch op.
        ...
        SetVisible(_shareLinesCheck, s2);
        SetVisible(_shareLinesHint, s2);
