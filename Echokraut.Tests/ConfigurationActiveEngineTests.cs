...
public class ConfigurationActiveEngineTests
{
    ...
    public void EchokrauTtsData_Defaults()
    {
        ...
        Assert.Null(d.TtsBackend); // legacy enum field: only ever set by an old config
        Assert.Equal("xtts", d.TtsEngine); // default: better quality
    ...
    [Theory]
    [InlineData("xtts", "xtts")]
    [InlineData("F5", "f5")]
    [InlineData("  moss  ", "moss")] // an engine the plugin has never heard of still round-trips
    [InlineData("", "xtts")]         // blank falls back to the default rather than an empty arg
    public void TtsBackendArg_IsTheNormalizedEngineId(string engine, string expectedArg)
    {
        var d = new EchokrauTtsData { TtsEngine = engine };
    ...
    public void MigrateTtsEngineId_MovesTheLegacyEnumToTheEngineId(EchokrauTtsEngine legacy, string expectedId)
    {
        var config = new Configuration { EchokrauTts = new EchokrauTtsData { TtsBackend = legacy } };

        config.MigrateTtsEngineId();

        Assert.Equal(expectedId, config.EchokrauTts.TtsEngine);
        Assert.Null(config.EchokrauTts.TtsBackend); // cleared, so the migration cannot run twice
    }

    [Fact]
    public void MigrateTtsEngineId_LeavesAnAlreadyMigratedChoiceAlone()
    {
        var config = new Configuration { EchokrauTts = new EchokrauTtsData { TtsEngine = "moss" } };

        config.MigrateTtsEngineId();

        Assert.Equal("moss", config.EchokrauTts.TtsEngine);
    }
    ...
    // ── the default root moved to C:\LocalTTS ───────────────────────────────────

    [Fact]
    public void DefaultInstallRoot_IsTheCurrentDefault()
    {
        Assert.Equal(Configuration.DefaultTtsInstallRoot, new Configuration().TtsInstallRoot);
        Assert.NotEqual(Configuration.LegacyDefaultTtsInstallRoot, Configuration.DefaultTtsInstallRoot);
    }

    [Fact]
    public void IsDefaultInstallRoot_AcceptsBothShippedDefaults()
    {
        // The migration asks "untouched default?", and after the move there are two right answers.
        Assert.True(Configuration.IsDefaultTtsInstallRoot(Configuration.DefaultTtsInstallRoot));
        Assert.True(Configuration.IsDefaultTtsInstallRoot(Configuration.LegacyDefaultTtsInstallRoot));
        Assert.False(Configuration.IsDefaultTtsInstallRoot(@"D:\my-tts"));
        Assert.False(Configuration.IsDefaultTtsInstallRoot(null));
    }

    [Fact]
    public void Migrate_FreshInstall_IsNotDraggedOntoTheLegacyRoot()
    {
        // The regression this guards: AlltalkData.LocalInstallPath still initialises to the OLD
        // default. Compared against the CURRENT default only, that untouched initialiser reads as a
        // "custom" legacy root — and every new user would silently land on C:\alltalk_tts.
        var cfg = new Configuration();
        Assert.Equal(Configuration.LegacyDefaultTtsInstallRoot, cfg.Alltalk.LocalInstallPath);
    ...
    [Fact]
    public void Migrate_ExistingInstallOnLegacyRoot_KeepsIt()
    {
        // An install made before the move: its files really are under the old root, so the move must
        // not relocate it.
        var cfg = new Configuration { TtsInstallRoot = Configuration.LegacyDefaultTtsInstallRoot };
        cfg.Alltalk.LocalInstallPath = Configuration.LegacyDefaultTtsInstallRoot;

        cfg.MigrateTtsInstallRoot();

        Assert.Equal(Configuration.LegacyDefaultTtsInstallRoot, cfg.TtsInstallRoot);
    }

    [Fact]
    public void Migrate_LegacyRootWithCustomLegacyPath_StillAdoptsTheCustomPath()
    {
        // Sitting on the old default is not a deliberate choice, so a real custom legacy path still
        // wins — the original point of the migration, preserved across the move.
        var cfg = new Configuration { TtsInstallRoot = Configuration.LegacyDefaultTtsInstallRoot };
        cfg.Alltalk.LocalInstallPath = @"D:\my-tts";
    ...
    [Fact]
    public void DefaultLocalSaveLocation_FollowsTheDefaultRoot()
    {
        // It used to hard-code the old root separately, so moving the default would have left it
        // pointing into a folder nothing else uses.
        Assert.StartsWith(Configuration.DefaultTtsInstallRoot, new Configuration().LocalSaveLocation);
    }
}
