...
public class EchokrauTtsDataTests
{
    ...
    [Fact]
    public void InstalledWrapperVersion_DefaultsToEmpty()
    {
        // Empty means "unknown", which WrapperUpdatePolicy treats as update-available — that is what
        // makes pre-handshake installs pick up the current wrapper instead of staying stale forever.
        Assert.Equal("", new EchokrauTtsData().InstalledWrapperVersion);
    }
    ...
    [InlineData("f5", "f5")]
    public void TtsBackendArg_IsLowerCased(string engine, string expected)
    {
        var data = new EchokrauTtsData { TtsEngine = engine };
