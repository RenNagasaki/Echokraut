...
public class RemoteUrlsReachabilityTests : IDisposable
{
    ...
    [Fact]
    public async Task EchokrauTtsReleasesUrl_ReturnsAReleaseTheUpdateCheckCanUse()
    {
        // Validates the whole "Check for updates" chain against the real repo: the API URL points at
        // a public repo, that repo HAS a release, and its assets include a zip our parser accepts.
        // A broken link here means the button reports a failure to every user.
        Assert.False(string.IsNullOrWhiteSpace(_data.EchokrauTtsReleasesUrl), "echokrauTtsReleasesUrl is missing");

        var request = new HttpRequestMessage(HttpMethod.Get, _data.EchokrauTtsReleasesUrl);
        request.Headers.TryAddWithoutValidation("User-Agent", "Echokraut");   // GitHub rejects requests without one
        request.Headers.TryAddWithoutValidation("Accept", "application/vnd.github+json");
        var response = await SendWithRetry(request);

        // Unauthenticated GitHub API calls are capped at 60/h per IP. Being rate-limited says nothing
        // about the URL being correct, so it must not fail the build — but it also cannot confirm it.
        if (response.StatusCode is HttpStatusCode.Forbidden or HttpStatusCode.TooManyRequests)
            return;

        Assert.True(response.IsSuccessStatusCode,
            $"Releases API returned {response.StatusCode}: {_data.EchokrauTtsReleasesUrl}");

        var body = await response.Content.ReadAsStringAsync();
        Assert.True(Echokraut.Helper.Functional.GitHubReleaseParser.TryParseLatestRelease(body, out var release, out var reason),
            $"Latest release is not usable by the update check: {reason}");
        Assert.False(string.IsNullOrWhiteSpace(release!.Tag));
        Assert.False(string.IsNullOrWhiteSpace(release.DownloadUrl));
    }
