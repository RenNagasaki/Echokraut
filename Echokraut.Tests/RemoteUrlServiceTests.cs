...
public class RemoteUrlServiceTests
{
    ...
    [Fact]
    public void FetchUrls_Success_PreservesTheEngineCatalogUrl()
    {
        // Same class of regression as above: a field missing from MergeWithFallback silently loses
        // the remote engine list and pins every user to the engines embedded in this plugin build.
        var json = MakeValidJson(d => d.EchokrauTtsEnginesUrl = "https://example.com/{version}/engines.json");
        using var http = MakeHttpClient(HttpStatusCode.OK, json);

        var service = new RemoteUrlService(_logMock.Object, http);

        Assert.Equal("https://example.com/{version}/engines.json", service.Urls.EchokrauTtsEnginesUrl);
    }

    [Fact]
    public void FetchUrls_EmptyEngineCatalogUrl_FallsBackToTheShippedOne()
    {
        var json = MakeValidJson(d => d.EchokrauTtsEnginesUrl = "");
        using var http = MakeHttpClient(HttpStatusCode.OK, json);

        var service = new RemoteUrlService(_logMock.Object, http);

        Assert.Equal(MakeFallback().EchokrauTtsEnginesUrl, service.Urls.EchokrauTtsEnginesUrl);
    }
    ...
    [Fact]
    public void FetchUrls_Success_PreservesEchokrauTtsVersion()
    {
        // Same class of regression as the two fields above: a field missing from MergeWithFallback
        // silently arrives empty, which here would permanently disable the wrapper Update offer.
        var json = MakeValidJson(d => d.EchokrauTtsVersion = "0.0.0.3");
        using var http = MakeHttpClient(HttpStatusCode.OK, json);

        var service = new RemoteUrlService(_logMock.Object, http);

        Assert.Equal("0.0.0.3", service.Urls.EchokrauTtsVersion);
    }

    [Fact]
    public void FetchUrls_EmptyEchokrauTtsVersion_FallsBackToEmbedded()
    {
        var json = MakeValidJson(d => d.EchokrauTtsVersion = "");
        using var http = MakeHttpClient(HttpStatusCode.OK, json);

        var service = new RemoteUrlService(_logMock.Object, http);

        Assert.Equal(MakeFallback().EchokrauTtsVersion, service.Urls.EchokrauTtsVersion);
    }
