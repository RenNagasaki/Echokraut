...
public class WrapperUpdatePolicyTests
{
    ...
    [Theory]
    [InlineData("0.0.0.3", "0.0.0.3", true)]  // the release that introduced it does have it
    [InlineData("0.0.0.4", "0.0.0.3", true)]
    [InlineData("0.0.0.2", "0.0.0.3", false)]
    [InlineData("0.10.0.0", "0.9.9.9", true)] // numeric per segment, not lexicographic
    [InlineData("0.9.9.9", "0.10.0.0", false)]
    [InlineData("0.1", "0.1.0.0", true)]      // missing segments count as 0
    [InlineData("1", "0.9.9.9", true)]
    public void IsAtLeast_ComparesTagsSegmentWise(string installed, string required, bool expected)
    {
        Assert.Equal(expected, WrapperUpdatePolicy.IsAtLeast(installed, required));
    }

    [Theory]
    [InlineData(null, "0.0.0.3")]      // unknown install
    [InlineData("", "0.0.0.3")]
    [InlineData("nightly", "0.0.0.3")] // tag scheme we do not understand
    [InlineData("0.0.0.2", "")]        // engine without a declared minimum
    [InlineData("0.0.0.2", null)]
    public void IsAtLeast_UnparseableEitherSide_DoesNotHideTheEngine(string? installed, string? required)
    {
        // A wrong "not available yet" silently removes a working choice — the more expensive mistake.
        Assert.True(WrapperUpdatePolicy.IsAtLeast(installed, required));
    }
}
