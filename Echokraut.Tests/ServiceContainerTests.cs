...
public class ServiceContainerTests
{
    ...
    [Fact]
    public void GetService_FactoryRegistration_RunsOnceAndCachesTheInstance()
    {
        // Factory registrations must behave as lazy singletons, not per-call construction.
        // Services that carry shared state — IDialogStateService being the obvious one — are
        // registered this way, and a fresh instance per resolve would silently split that state
        // between the addon hooks, the speech pipeline and the toolbar.
        var container = new ServiceContainer();
        var built = 0;
        container.RegisterFactory<ITestService>(_ => { built++; return new ConcreteService(); });
