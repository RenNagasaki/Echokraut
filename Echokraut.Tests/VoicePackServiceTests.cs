...
public class VoicePackServiceTests : IDisposable
{
        ...
        : HttpMessageHandler
    {
        ...
        /// <summary>The URL of the last request — this is how the per-language pack choice is
        /// asserted, since the choice is only visible in what gets fetched.</summary>
    ...
    private static Mock<IRemoteUrlService> MakeUrls(string voicePackUrl,
        Dictionary<string, string>? perLanguage = null)
    {
        var mock = new Mock<IRemoteUrlService>();
        mock.SetupGet(u => u.Urls).Returns(new RemoteUrlsData
        {
            VoicePackUrl = voicePackUrl,
            VoicePackUrls = perLanguage ?? new Dictionary<string, string>(),
        });
        return mock;
    }
        ...
        HttpStatusCode status = HttpStatusCode.OK)
    {
        ...
        return new VoicePackService(_log.Object, config, MakeUrls(url).Object,
            () => ClientLanguage.English, http);
    }
    ...
    public async Task DownloadAsync_WithoutConfiguredUrl_SkipsWithoutTouchingDisk()
    {
        ...
        var sut = new VoicePackService(_log.Object, config, MakeUrls(string.Empty).Object,
            () => ClientLanguage.English, new HttpClient(handler));
    ...
    [Fact]
    public async Task DownloadAsync_GermanClient_FetchesTheGermanPack()
    {
        var handler = new StubHandler(MakeZip(("Male_All_NPC001.wav", "wav")));
        var config = new Configuration { LocalSaveLocation = _tempRoot };
        var urls = MakeUrls("https://example.invalid/english.zip", new Dictionary<string, string>
        {
            ["English"] = "https://example.invalid/english.zip",
            ["German"] = "https://example.invalid/german.zip",
        });
        var sut = new VoicePackService(_log.Object, config, urls.Object,
            () => ClientLanguage.German, new HttpClient(handler));

        await sut.DownloadAsync(CancellationToken.None);

        Assert.Equal("https://example.invalid/german.zip", handler.LastUrl);
    }

    [Fact]
    public async Task DownloadAsync_LanguageWithoutItsOwnPack_FallsBackToEnglish()
    {
        var handler = new StubHandler(MakeZip(("Male_All_NPC001.wav", "wav")));
        var config = new Configuration { LocalSaveLocation = _tempRoot };
        var urls = MakeUrls(string.Empty, new Dictionary<string, string>
        {
            ["English"] = "https://example.invalid/english.zip",
        });
        var sut = new VoicePackService(_log.Object, config, urls.Object,
            () => ClientLanguage.French, new HttpClient(handler));

        await sut.DownloadAsync(CancellationToken.None);

        Assert.Equal("https://example.invalid/english.zip", handler.LastUrl);
    }
}
