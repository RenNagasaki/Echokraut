...
public class DatabaseServiceTests : IDisposable
{
    ...
    [Fact]
    public void GetPhoneticCorrectionModels_StaysInSyncWithTheEntities()
    {
        // The live speech path reads the projected models for every spoken line, so the cache
        // behind them has to track every write — a stale projection would silently apply the
        // wrong pronunciation until the next plugin start.
        _db.UpsertPhoneticCorrection("Miqo'te", "Mee-koh-teh");

        var models = _db.GetPhoneticCorrectionModels();
        Assert.Single(models);
        Assert.Equal("Miqo'te", models[0].OriginalText);
        Assert.Equal("Mee-koh-teh", models[0].CorrectedText);

        _db.UpsertPhoneticCorrection("Miqo'te", "Mee-ko-tay");
        Assert.Equal("Mee-ko-tay", _db.GetPhoneticCorrectionModels()[0].CorrectedText);

        _db.DeletePhoneticCorrection("Miqo'te");
        Assert.Empty(_db.GetPhoneticCorrectionModels());
    }
