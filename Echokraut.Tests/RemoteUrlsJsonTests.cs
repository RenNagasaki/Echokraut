...
public class RemoteUrlsJsonTests
{
    ...
    [Fact]
    public void JsonFile_EchokrauTtsVersionMatchesTheTagInTheWrapperUrl()
    {
        // The wrapper Update button compares the installed tag against echokrauTtsVersion, while the
        // download uses echokrauTtsUrl. Bumping one without the other either offers an update that
        // reinstalls the same build, or silently ships a new wrapper nobody is offered.
        var data = LoadFromFile();

        Assert.False(string.IsNullOrWhiteSpace(data.EchokrauTtsUrl), "echokrauTtsUrl is missing");
        Assert.False(string.IsNullOrWhiteSpace(data.EchokrauTtsVersion), "echokrauTtsVersion is missing");
        Assert.Contains($"/{data.EchokrauTtsVersion}/", data.EchokrauTtsUrl);
    }

    [Fact]
    public void JsonFile_HasAReleasesApiUrlForTheUpdateCheck()
    {
        // Without it the "Check for updates" button can only report a configuration error.
        var data = LoadFromFile();

        Assert.False(string.IsNullOrWhiteSpace(data.EchokrauTtsReleasesUrl), "echokrauTtsReleasesUrl is missing");
        Assert.True(Uri.IsWellFormedUriString(data.EchokrauTtsReleasesUrl, UriKind.Absolute));
    }

    [Fact]
    public void JsonFile_HasAnEngineCatalogUrlWithAVersionPlaceholder()
    {
        // The placeholder is what pins the engine list to the wrapper release users can install.
        // Without it every user reads whatever the wrapper's default branch happens to hold.
        var data = LoadFromFile();

        Assert.False(string.IsNullOrWhiteSpace(data.EchokrauTtsEnginesUrl), "echokrauTtsEnginesUrl is missing");
        Assert.Contains("{version}", data.EchokrauTtsEnginesUrl);
    }
