...
- **Caveat to document**: AllTalk→EchokrauTTS copies `.wav` without reference transcripts; F5-TTS
  then relies on ASR/`ref_text` fallback (lower quality). The downloaded voice pack ships a `.txt`
  transcript next to every sample, so packs installed from it are unaffected.
