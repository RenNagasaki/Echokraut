...
> (AllTalk ↔ EchokrauTTS, and between the sub-engines inside EchokrauTTS).

## Disclaimer

- Echokraut supports **two TTS engines** — **EchokrauTTS** (the default) and **AllTalk_TTS** — each usable in **Local** (runs on your own machine), **Remote** (connect to a server), or **Audio Files Only** (no generation) mode.
- **Hardware:** most local sub-engines are GPU-based and want an Nvidia GPU on Windows (AMD is supported on Linux), 6 GB+ VRAM recommended — but **a GPU is no longer a hard requirement**: EchokrauTTS ships a small CPU sub-engine (MOSS) that leaves your graphics card to the game. It is quicker to start speaking and a much smaller download, at noticeably lower audio quality. Remote mode offloads generation to another machine entirely.
...
- **Voice Clip Manager** (`/ek`) — Browse every line you have collected per NPC, replay it, regenerate it or delete it.

### Community & Voices

- **Voice pack** — A curated pack of freely-licensed reference voices is downloaded on install. **Echokraut never reads, decodes or redistributes audio from your game installation.** Packs can be published per game language; a language without its own pack gets the English one.
- **Community dialogue database** — While you talk to NPCs, Echokraut reports the dialogue line, its language and which NPC says it, so lines missing from the plugin's data can be found and added. Your character name is replaced by a placeholder, nothing about you is sent, and chat is never included. On by default, switchable off under Settings → General.
...
- **Native FFXIV UI** — Every window is a native FFXIV-styled addon (powered by KamiToolKit), not an overlay.
...
- **EchokrauTTS** (default) — [EchokrauTTS](https://github.com/RenNagasaki/Echokrautts), the purpose-built engine for Echokraut. Available in Local and Remote modes. A local install ships **several sub-engines** (XTTS, F5 and the CPU-friendly MOSS at the time of writing) and installs all of them, so switching is a restart, never a reinstall. **The plugin reads the list of sub-engines from the EchokrauTTS release itself**, so an engine added by a new EchokrauTTS version shows up without a plugin update — listed with the version it needs if your installed wrapper is older.
